#!/usr/bin/env python3
"""
Générateur d'audio — module « Des nouvelles à donner » (module-relations).
...
`build/contenu/module-relations/dialogues.js`, la source unique.
...
`build/collecte_sons.py`, qui écrit `sons_module_relations.json`.

Usage :  python3 generer_audio_module_relations.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
MANIFESTE = RACINE / "sons_module_relations.json"
DIALOGUES_JS = RACINE / "build/contenu/module-relations/dialogues.js"
...
# Quatre personnages pour quatre voix. Chantal et Fatou partagent la voix
# féminine #1 : elles ne se répondent jamais — Fatou n'apparaît que dans son
# message vocal, où elle est seule à parler.
VOIX_PERSO = {
    "MARIAMA": "feminin_2",
    "CHANTAL": "enseignante",
    "FATOU":   "enseignante",
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"lancer build/collecte_sons.py module-relations")
        sys.exit(1)
    ...
    print(f"🔊 module-relations — {len(taches)} extraits "
