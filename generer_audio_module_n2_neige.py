#!/usr/bin/env python3
"""
Générateur d'audio — module « Il fait froid, je m'habille » (niveau 2) (module-n2-neige).
...
`build/contenu/module-n2-neige/dialogues.js`, la source unique.

Les identifiants de la famille 2, eux, ne s'inventent toujours pas : ils
viennent du moteur. **Ils se relèvent hors navigateur**, par
`node build/releve_sons.js module-n2-neige > sons_module_n2_neige.json`.
...
21 août 2026 — un port déjà pris par une session voisine, puis un serveur
oublié en tâche de fond.

**Écrit le 21 août 2026 au soir, sans avoir jamais tourné.** Le compte
ElevenLabs était épuisé ce jour-là (`401 quota_exceeded`) et devait être
rechargé le lendemain. Le contenu, lui, était fini : le relevé
`sons_module_n2_neige.json` est complet et commité, et ce script est prêt à
produire les **234 extraits** — 57 répliques sur six dialogues, 177 sons — en
une seule passe. Rien n'est à préparer avant de le lancer : il saute ce qui
existe déjà, donc une reprise après coupure ne coûte rien.

Usage :  python3 generer_audio_module_n2_neige.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n2-neige"
MANIFESTE = RACINE / "sons_module_n2_neige.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n2-neige/dialogues.js"
...
# Quatre personnages pour quatre voix. Zina prend celle de l'enseignante, la
# seule que `voix_lente` ralentit à 0,85 : c'est elle qui répète la
# température et qui explique « moins vingt » à son fils, et un nombre mal
# entendu envoie l'élève dehors avec le mauvais manteau. Roland, le voisin
# retraité, et la voix de la radio gardent deux voix masculines distinctes —
# le bulletin de sept heures ne doit jamais se confondre avec quelqu'un qui
# parle dans l'entrée de l'immeuble. Youssef, neuf ans, prend la seconde voix
# féminine : à cet âge, la voix n'a pas encore mué, et c'est le seul timbre
# clair dont le générateur dispose.
VOIX_PERSO = {
    "ZINA":     "enseignante",
    "ROLAND":   "masculin_1",
    "VOIX":     "narrateur",
...
def main():
    ...
    print(f"🔊 module-n2-neige — {len(taches)} extraits "
