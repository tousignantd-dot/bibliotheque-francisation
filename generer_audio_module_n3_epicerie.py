#!/usr/bin/env python3
"""
Générateur d'audio — module « À l'épicerie » (niveau 3) (module-n3-epicerie).
...
`build/contenu/module-n3-epicerie/dialogues.js`, la source unique.
...
`build/collecte_sons.py`, qui écrit `sons_module_n3_epicerie.json`.

Usage :  python3 generer_audio_module_n3_epicerie.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n3-epicerie"
MANIFESTE = RACINE / "sons_module_n3_epicerie.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n3-epicerie/dialogues.js"
...
# Quatre personnages pour quatre voix. Stéphane, à la caisse, prend la voix du
# narrateur : il n'apparaît qu'au défi 3, où Éric n'est pas.
VOIX_PERSO = {
    "MARISOL": "feminin_2",
    "ÉRIC":    "masculin_1",
    "GINETTE": "enseignante",
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"lancer build/collecte_sons.py module-n3-epicerie")
        sys.exit(1)
    ...
    print(f"🔊 module-n3-epicerie — {len(taches)} extraits "
