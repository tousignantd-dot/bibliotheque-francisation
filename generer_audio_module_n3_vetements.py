#!/usr/bin/env python3
"""
Générateur d'audio — module « Magasiner du linge » (module-n3-vetements, niveau 3).
...
Les dialogues sont lus dans `build/contenu/module-n3-vetements/dialogues.js` :
une seule source, comme aux niveaux 2 et 5.

Les identifiants de la famille 2 ne s'inventent pas — ils sont produits par le
moteur au rendu. `sons_module_n3_vetements.json` a été produit hors navigateur
par `/tmp/releve_vet.js`, qui rejoue sur les fichiers de contenu les trois
endroits du gabarit qui appellent `playWord` : le bloc `savoir`
...
mini-leçons. Le relevé a été comparé à celui obtenu dans le navigateur : même
nombre de clés, mêmes valeurs. Le détour s'imposait — la page ouverte en
`data:` ne peut pas écrire dans `localStorage`, donc `render()` y échoue et le
relevé par le DOM rend zéro pastille de mot.
...
Usage :  python3 generer_audio_module_n3_vetements.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n3-vetements"
MANIFESTE = RACINE / "sons_module_n3_vetements.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n3-vetements/dialogues.js"
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"lancer build/collecte_sons.py module-n3-vetements")
        sys.exit(1)
    ...
    print(f"🔊 module-n3-vetements — {len(taches)} extraits "
