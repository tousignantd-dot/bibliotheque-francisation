#!/usr/bin/env python3
"""
Générateur d'audio — module « Une nuit à l'urgence »
(module-n5-urgence, niveau 5, activité 66).
...
Les dialogues sont lus dans `build/contenu/module-n5-urgence/dialogues.js` :
une seule source.

Les identifiants de la famille 2 ne s'inventent pas — ils sont produits par le
moteur au moment du rendu. `sons_module_n5_urgence.json` a été relevé **hors
navigateur**, en rejouant en vingt lignes de node les quatre endroits du
...
Deux avertissements qui viennent des modules précédents et qui valent ici :
...
- **`build/collecte_sons.py` n'a pas été lancé, et ne doit pas l'être.** Il
  ...
  complet une fois les MP3 payés.
- **Tout bloc `ana` porte son champ `say:`.** Sans lui, le moteur concatène
  ...
  de tableau à voix haute. Ça ne se découvre qu'une fois les MP3 payés.
...
Usage :  python3 generer_audio_module_n5_urgence.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n5-urgence"
MANIFESTE = RACINE / "sons_module_n5_urgence.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n5-urgence/dialogues.js"
...
# Sept locuteurs, trois voix — et la règle tenue partout : deux personnages ne
# partagent une voix que s'ils ne se répondent jamais dans un même dialogue.
# Marisol traverse les quatre dialogues, donc elle garde la sienne. Amparo (Je
# découvre), Karine au triage (défi 2) et la docteure Lamontagne (défi 3) se
# partagent la voix « enseignante », ralentie à 0,85 : elles ne se croisent
# jamais, et cette voix plus lente est un avantage pour les deux soignantes,
# que l'élève doit arriver à suivre. Patrick au 8-1-1, Vincent au 9-1-1 et
# Julio prennent la voix masculine, chacun dans un dialogue différent.
VOIX_PERSO = {
    "MARISOL":         "feminin_2",
    "AMPARO":          "enseignante",
    "KARINE":          "enseignante",
    "DRE LAMONTAGNE":  "enseignante",
    "PATRICK":         "masculin_1",
    "VINCENT":         "masculin_1",
    "JULIO":           "masculin_1",
}
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"lancer build/collecte_sons.py module-n5-urgence")
        sys.exit(1)
    ...
    print(f"🔊 module-n5-urgence — {len(taches)} extraits "
