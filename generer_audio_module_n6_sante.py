"""
Générateur d'audio — module « Nommer ce qu'on a »
(module-n6-sante, niveau 6, activité 104).
...
**292 extraits attendus** : 85 répliques sur quatre dialogues, et 207 sons.

Les dialogues sont lus dans `build/contenu/module-n6-sante/dialogues.js` :
...
moteur au rendu. `sons_module_n6_sante.json` a été produit **hors navigateur**,
par :

    node build/releve_sons.js module-n6-sante > sons_module_n6_sante.json
...
Les dialogues sont longs — vingt à vingt-trois répliques chacun, dont
plusieurs de cinquante mots. Une conversation de salle d'attente qui dure deux
heures et un entretien de spécialité ne se découpent pas en saynètes de trois
répliques : la compétence du niveau vise « des discours détaillés et
structurés ». Le coût par extrait est donc plus élevé qu'aux niveaux 2 et 3 à
nombre d'extraits égal.
...
shampoing » : le relevé rend `prGraphie_gr12 → un short`, pas une phrase
...
**Cinq voix pour quatre timbres** — deux personnages ne partagent un timbre
que s'ils ne se répondent jamais, c'est vérifié :

    prep  LEYLA, MARIETTE          t1  LEYLA, GILLES
    t2    LEYLA, SYLVINE           t3  LEYLA, PIERRE-LUC

MARIETTE, l'agente administrative de l'accueil, et SYLVINE, l'interniste,
partagent `enseignante` sans jamais se croiser : elles n'apparaissent pas dans
le même dialogue et ne se répondent nulle part. C'est un choix et non un
pis-aller — les deux tiennent un discours d'explication, où un débit posé est
un avantage. La voix « enseignante » est ralentie à 0,85 par `voix_lente.py` ;
le paramètre `speed` d'ElevenLabs ne fonctionne pas avec
`eleven_multilingual_v2`. LEYLA, GILLES et PIERRE-LUC gardent un débit
...
muet, et le son se rattrape d'un seul coup :
    ...
    python3 generer_audio_module_n6_sante.py
...
Usage :  python3 generer_audio_module_n6_sante.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n6-sante"
MANIFESTE = RACINE / "sons_module_n6_sante.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n6-sante/dialogues.js"
...
# Cinq personnages pour quatre timbres. Deux peuvent partager une voix
# seulement s'ils ne se répondent jamais dans un même dialogue ; c'est
# vérifié dans le docstring.
VOIX_PERSO = {
    "LEYLA":       "feminin_2",
    "GILLES":      "masculin_1",
    "MARIETTE":    "enseignante",
    "SYLVINE":     "enseignante",
    "PIERRE-LUC":  "narrateur",
}
...
def parle(cle, texte, voix, chemin):
    ...
    tentatives. Sans cela, une série de deux cent cinquante extraits s'arrête
    au milieu, sur la trace d'une exception, et personne n'est là pour la
    relancer.
    """
    for essai in range(1, ESSAIS + 1):
        try:
            ...
            # secondes. On garde le doublement, sur une base plus courte.
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — le produire par "
              f"« node build/releve_sons.js module-n6-sante > "
    ...
    print(f"🔊 module-n6-sante — {len(taches)} extraits "
