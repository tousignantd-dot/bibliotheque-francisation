#!/usr/bin/env python3
"""
Générateur d'audio — module « Je remplis ma fiche » (niveau 1)
...
`build/contenu/module-n1-inscription/dialogues.js`, la source unique. Les
générateurs qui en gardaient une copie divergeaient du module à la première
correction.

Les identifiants de la famille 2, eux, ne s'inventent pas : ils viennent du
moteur. **Ils se relèvent hors navigateur**, par

    node build/releve_sons.js module-n1-inscription > sons_module_n1_inscription.json
...
Quatre points propres à ce module
---------------------------------
- **Les extraits sont très courts.** C'est un module de niveau 1 : les
  pastilles disent « treize », « trente », « une case ». `voix.charge_utile()`
  ajoute donc un contexte français **non prononcé** avant et après chaque
  extrait de quatre mots ou moins — sans lui, « case », « fiche » et
  « adresse » ressortent régulièrement à l'anglaise. Le mot sort seul dans le
  fichier ; seul le modèle lit le contexte.
- **Six extraits sont des nombres qui se distinguent par leur fin** — treize
  et trente, quatorze et quarante, quinze et cinquante. C'est le contenu même
  de l'exercice `prSons` : ces MP3 doivent être écoutés avant d'être publiés,
  parce qu'une synthèse qui avale la dernière syllabe rendrait l'exercice
  impossible plutôt que difficile.
- **Les clés de `CARRIER_PHRASES` sont les mots accentués**, tels qu'ils
  paraissent dans les listes `savoir[…][2]` de `exos.js` — articles et
  apostrophes compris (« l'adresse », « l'année », « le nom de famille »).
  Vérifié par `node build/coherence.js module-n1-inscription`.
- **Un extrait épelle un courriel** : « yusuf point daoud, arobase, courriel
  point c a ». Les mots « point » et « arobase » sont écrits en toutes
  lettres dans le dialogue, jamais en signes — la synthèse dirait « at » ou
  se tairait.

`parle()` réessaie cinq fois. Une coupure TLS n'est pas un refus de débit :
la liaison revient d'elle-même en quelques secondes, d'où deux bases
d'attente distinctes.

Usage :  python3 generer_audio_module_n1_inscription.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n1-inscription"
MANIFESTE = RACINE / "sons_module_n1_inscription.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n1-inscription/dialogues.js"
...
# Trois personnages. Madame Côté prend la voix de l'enseignante, la seule que
# `voix_lente` ralentit à 0,85 : c'est elle qui demande les cases de la fiche
# une à une, et une case mal entendue fait écrire la mauvaise réponse. Yusuf
# et Carlos gardent deux voix masculines distinctes — ils se répondent dans le
# dialogue `t2`.
VOIX_PERSO = {
    "MADAME CÔTÉ": "enseignante",
    "YUSUF":       "masculin_1",
    "CARLOS":      "narrateur",
}
...
def slug(nom):
    ...
    normaliserait tout : charSlug garde le trait d'union.
    """
...
def parle(cle, texte, voix, chemin, avant=None, apres=None):
    ...
    for essai in range(1, ESSAIS + 1):
        try:
            ...
            # secondes. Le 429 et les 5xx, eux, gardent l'attente longue plus
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"node build/releve_sons.js module-n1-inscription > {MANIFESTE.name}")
        sys.exit(1)
    ...
    print(f"🔊 module-n1-inscription — {len(taches)} extraits "
