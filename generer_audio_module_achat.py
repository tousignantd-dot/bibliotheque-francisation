#!/usr/bin/env python3
"""
Générateur d'audio — module « Acheter un appareil » (module-achat).
...
`build/contenu/module-achat/dialogues.js`, la source unique.
...
`build/collecte_sons.py`, qui écrit `sons_module_achat.json`.

Usage :  python3 generer_audio_module_achat.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-achat"
MANIFESTE = RACINE / "sons_module_achat.json"
DIALOGUES_JS = RACINE / "build/contenu/module-achat/dialogues.js"
...
# Quatre personnages pour quatre voix. Patrick, le livreur, prend la voix du
# narrateur : il n'apparaît que dans son appel de confirmation, seul.
VOIX_PERSO = {
    "YIN":     "feminin_2",
    "THÉRÈSE": "enseignante",
    "MARTIN":  "masculin_1",
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"lancer build/collecte_sons.py module-achat")
        sys.exit(1)
    ...
    print(f"🔊 module-achat — {len(taches)} extraits "
