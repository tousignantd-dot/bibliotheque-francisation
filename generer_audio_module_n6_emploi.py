"""
Générateur d'audio — module « Le poste affiché à l'interne »
(module-n6-emploi, niveau 6, activité 100).
...
**290 extraits attendus** : 84 répliques sur quatre dialogues, et 206 sons.

Les dialogues sont lus dans `build/contenu/module-n6-emploi/dialogues.js` :
une seule source, comme aux niveaux 2, 3 et 5.

Les identifiants de la famille 2 ne s'inventent pas — ils sont produits par le
moteur au rendu. `sons_module_n6_emploi.json` a été produit **hors
navigateur**, par :

    node build/releve_sons.js module-n6-emploi > sons_module_n6_emploi.json
...
Les extraits sont longs : dix-huit à vingt-trois répliques par dialogue, dont
plusieurs de quarante mots — une explication de démarche en cinq étapes et une
rencontre d'information ne se découpent pas en saynètes de trois répliques. Le
coût par extrait est donc plus élevé qu'aux niveaux 2 et 3, à nombre
d'extraits égal.

**Les mots isolés passent par `charge_utile()` de `build/voix.py`**, qui
donne aussi aux répliques de dialogue leurs voisines de scène.** Ce module est
plein de mots courts que l'anglais connaît aussi — « six », « dix »,
« un short », « un t-shirt », « un shampoing » — et l'exercice de
graphie-phonie les envoie **seuls** à la synthèse : le relevé rend
`prGraphie_gr11 → un t-shirt`, pas la phrase porteuse. Sans contexte français,
« un t-shirt » sort à l'anglaise, et c'est justement le mot dont l'élève doit
entendre la prononciation française. `charge_utile()` pose `previous_text` et
`next_text` autour de tout extrait de quatre mots ou moins ; rien de ce
contexte n'est prononcé ni facturé. Le pilote du niveau 6 a payé cette
découverte ; aucun module du niveau ne doit s'en passer.

**Les 28 phrases porteuses de `carrier.js` servent les pastilles des bandeaux
de savoir**, pas les cartes de l'exercice de graphie-phonie — le gabarit lit
`CARRIER_PHRASES[w]` pour les premières et le texte de la rangée pour les
secondes. Les seize premières clés sont donc lues, les douze de la
graphie-phonie ne le sont pas : elles restent pour mémoire, et
`node build/coherence.js` ne les compte pas comme un écart, à raison.

**Quatre personnages, quatre timbres** — chacun le sien, ce qui évite tout
risque de croisement :

    prep  YANETH, GHISLAIN                    t1  YANETH, MARIE-SOLEIL
    t2    YANETH, GHISLAIN                    t3  PATRICE, MARIE-SOLEIL,
                                                  GHISLAIN, YANETH

Le dialogue `t3` réunit les quatre à lui seul : c'est la rencontre
d'information, et c'est ce qui interdisait de faire partager une voix à deux
personnages. MARIE-SOLEIL prend la voix « enseignante », ralentie à 0,85 par
`voix_lente.py`, et c'est un choix, pas un défaut : c'est elle qui énonce la
démarche en cinq étapes que l'élève doit pouvoir suivre et redire, et un débit
posé sert cet extrait-là. Les trois autres gardent un débit normal — au
...
muet, et le son se rattrape d'un seul coup :
    ...
    python3 generer_audio_module_n6_emploi.py
...
Usage :  python3 generer_audio_module_n6_emploi.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n6-emploi"
MANIFESTE = RACINE / "sons_module_n6_emploi.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n6-emploi/dialogues.js"
...
# Quatre personnages, quatre timbres — aucun partage. Le dialogue `t3`, la
# rencontre d'information, les réunit tous les quatre : deux personnages ne
# peuvent donc pas se partager une voix ici, contrairement aux modules dont
# les dialogues n'ont jamais plus de deux locuteurs.
#
# MARIE-SOLEIL prend la voix « enseignante », ralentie à 0,85 par
# `voix_lente.py`. C'est un choix : c'est elle qui énonce la démarche en cinq
# étapes, celle que l'élève doit pouvoir suivre puis redire dans « Je me
# lance ». Les trois autres gardent un débit normal — au niveau 6, la
# compétence vise le débit normal.
VOIX_PERSO = {
    "YANETH":       "feminin_2",
    "GHISLAIN":     "masculin_1",
    "MARIE-SOLEIL": "enseignante",
    "PATRICE":      "narrateur",
}
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — le produire par "
              f"« node build/releve_sons.js module-n6-emploi > "
    ...
    print(f"🔊 module-n6-emploi — {len(taches)} extraits "
