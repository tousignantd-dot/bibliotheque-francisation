#!/usr/bin/env python3
"""
Générateur d'audio — module « Je retire de l'argent » (niveau 2) (module-n2-guichet).
...
`build/contenu/module-n2-guichet/dialogues.js`, la source unique.
...
`node build/releve_sons.js module-n2-guichet > sons_module_n2_guichet.json` —
...
Au 21 août 2026, ElevenLabs répond `401 quota_exceeded` : il ne reste aucun
crédit sur le compte. Les 242 extraits sont donc en attente, et ce script est
relançable tel quel dès que le quota est rétabli — il saute ce qui existe.

Usage :  python3 generer_audio_module_n2_guichet.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n2-guichet"
MANIFESTE = RACINE / "sons_module_n2_guichet.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n2-guichet/dialogues.js"
...
# Cinq personnages pour quatre voix. Monique, l'employée du centre sportif qui
# dicte le chèque ligne par ligne, prend la voix de l'enseignante — celle que
# `voix_lente` ralentit à 0,85 : c'est précisément elle qu'un élève de
# niveau 2 doit pouvoir suivre.
#
# L'écran du guichet et Claude Fontaine partagent la voix du narrateur. Ils ne
# se parlent jamais : l'écran ne dit ses six phrases que dans le dialogue
# `t1`, où Claude est absent, et Claude ne parle que dans `prep` et `t1b`, où
# l'écran ne dit rien.
VOIX_PERSO = {
    "AMADOU":   "masculin_1",
    "MONIQUE":  "enseignante",
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — lancer\n"
              f"   node build/releve_sons.js module-n2-guichet > {MANIFESTE.name}")
        sys.exit(1)
    ...
    print(f"🔊 module-n2-guichet — {len(taches)} extraits "
