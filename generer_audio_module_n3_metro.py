...
Les dialogues sont lus dans `build/contenu/module-n3-metro/dialogues.js` :
une seule source, comme aux niveaux 2 et 5.

Les identifiants de la famille 2 ne s'inventent pas — ils sont produits par le
moteur au rendu. `sons_module_n3_metro.json` a été produit hors navigateur
par `node build/releve_sons.js module-n3-metro`, qui rejoue sur les fichiers de
...
Usage :  python3 generer_audio_module_n3_metro.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
MANIFESTE = RACINE / "sons_module_n3_metro.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n3-metro/dialogues.js"
...
# moins un dialogue, donc aucune ne peut être partagée. Lorraine, la préposée
# du point de service, est celle qui explique les titres et les prix : elle
# prend la voix « enseignante », ralentie à 0,85 par voix_lente.py — ce qui
# tombe juste, puisque tout le Défi 1 consiste à retenir des chiffres dits à
# voix haute.
VOIX_PERSO = {
    "ROSARIO":  "feminin_2",
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"relancer node node build/releve_sons.js module-n3-metro")
        sys.exit(1)
    ...
    print(f"🔊 module-n3-metro — {len(taches)} extraits "
