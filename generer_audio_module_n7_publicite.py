#!/usr/bin/env python3
"""
Générateur d'audio — module « Ce que la publicité ne dit pas » (niveau 7)
(module-n7-publicite).
...
`build/contenu/module-n7-publicite/dialogues.js`, la source unique.

Niveau 7, donc des **discours longs** : les quatre extraits font de vingt à
vingt-cinq répliques. Le Défi 1 contient en plus deux capsules publicitaires
transcrites mot pour mot et dites par la voix « narrateur » — ce sont les deux
seules répliques du module qui ne s'adressent à personne, et tout l'exercice
d'écoute consiste à entendre que leur fin est deux fois plus rapide que leur
début. Le débit ne se règle pas ici : `voix_lente.py` ne ralentit que la voix
« enseignante », et l'annonceur n'en fait pas partie.

Le relevé des sons (famille 2) n'a **pas** été fait par le navigateur ni par
`build/collecte_sons.py`, qu'il ne faut pas lancer :

    node build/releve_sons.js module-n7-publicite > sons_module_n7_publicite.json
...
Usage :  python3 generer_audio_module_n7_publicite.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n7-publicite"
MANIFESTE = RACINE / "sons_module_n7_publicite.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n7-publicite/dialogues.js"
...
# Six locuteurs pour quatre voix. Deux ne peuvent partager une voix que s'ils
# ne se répondent jamais dans un même extrait ; c'est vérifié ici :
#
#   prep  YAMILÉ, RÉGINALD          t1  RÉGINALD, ANNONCEUR, YAMILÉ
#   t2    DORIANE, YAMILÉ           t3  VALERIA, YAMILÉ, MAXIME
#
# DORIANE et VALERIA partagent `enseignante` sans jamais se croiser ; RÉGINALD
# et MAXIME partagent `masculin_1`, eux non plus.
#
# **Le personnage de l'Office a été écrit au masculin pour cette raison-là, et
# c'est la seule contrainte de casting qu'un module de niveau 7 rencontre.**
# Le troisième extrait réunit la mère, sa fille et l'agent d'information : à
# trois femmes dans un même dialogue, deux voix féminines ne suffisent pas.
# Plutôt que de faire parler deux personnages de la même voix dans la même
# scène — ce que l'élève entend immédiatement —, Mylène Sarrazin est devenue
# Maxime Sarrazin. À retenir pour les modules suivants : compter les
# locuteurs par extrait AVANT d'écrire les dialogues, pas après.
#
# Le choix de qui prend `enseignante` n'est pas neutre : c'est la voix que
# `voix_lente` ralentit à 0,85. Elle va à Doriane, la conseillère budgétaire
# qui fait lire le dépliant ligne par ligne — exactement le rôle pour lequel
# un débit posé a été introduit — et à Valeria, onze ans, qui n'a que cinq
# répliques courtes.
#
# L'ANNONCEUR prend `narrateur`, et c'est délibéré : les deux capsules
# publicitaires ne s'adressent à personne dans la scène, et une voix distincte
# de celle des trois interlocuteurs est ce qui les fait entendre comme des
# enregistrements. Elle n'est pas ralentie — la vitesse de la mention légale
# EST l'objet de l'exercice.
VOIX_PERSO = {
    "YAMILÉ":     "feminin_2",
    "RÉGINALD":   "masculin_1",
    "ANNONCEUR":  "narrateur",
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"lancer build/collecte_sons.py module-n7-publicite")
        sys.exit(1)
    ...
    print(f"🔊 module-n7-publicite — {len(taches)} extraits "
