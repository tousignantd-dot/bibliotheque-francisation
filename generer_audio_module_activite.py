#!/usr/bin/env python3
"""
Générateur d'audio — module « S'inscrire à une activité » (module-activite).
...
`build/contenu/module-activite/dialogues.js`, la source unique.
...
`build/collecte_sons.py`, qui écrit `sons_module_activite.json`.

Usage :  python3 generer_audio_module_activite.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
MANIFESTE = RACINE / "sons_module_activite.json"
DIALOGUES_JS = RACINE / "build/contenu/module-activite/dialogues.js"
...
# Quatre personnages pour quatre voix. Denis prend la voix du narrateur : il
# n'apparaît qu'au bord de la piscine et dans le dialogue du dépliant, jamais
# avec Kevin.
VOIX_PERSO = {
    "ROSA":   "feminin_2",
    "LOUISE": "enseignante",
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"lancer build/collecte_sons.py module-activite")
        sys.exit(1)
