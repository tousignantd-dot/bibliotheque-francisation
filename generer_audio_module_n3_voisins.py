#!/usr/bin/env python3
"""
Générateur d'audio — module « L'escalier de la rue Dézéry » (module-n3-voisins,
niveau 3, activité 82).
...
**316 extraits attendus** : 72 répliques sur cinq dialogues, et 244 sons.

Les dialogues sont lus dans `build/contenu/module-n3-voisins/dialogues.js` :
une seule source, comme aux niveaux 2, 3 et 5.

Les identifiants de la famille 2 ne s'inventent pas — ils sont produits par le
moteur au rendu. `sons_module_n3_voisins.json` a été produit **hors
navigateur**, par :

    node build/releve_sons.js module-n3-voisins > sons_module_n3_voisins.json
...
jamais trouvée — la pastille lirait alors le mot seul, mal prononcé. Les 57
clés de ce module ont été relevées par script, et `node build/coherence.js
module-n3-voisins` le revérifie dans les deux sens.

**État au 21 août 2026 : aucun extrait n'a encore été produit.** ElevenLabs
répond `401 quota_exceeded`, crédits à zéro. Le module est livré muet, et le
son se rattrape d'un seul coup dès que le compte est rechargé :

    python3 generer_audio_module_n3_voisins.py
...
Usage :  python3 generer_audio_module_n3_voisins.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n3-voisins"
MANIFESTE = RACINE / "sons_module_n3_voisins.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n3-voisins/dialogues.js"
...
# Trois personnages, trois voix distinctes : Manon, Rachid et Leïla se
# répondent dans le même dialogue d'ouverture, donc aucune ne peut être
# partagée. Manon garde la voix de l'enseignante — c'est elle qui porte le
# module, et c'est aussi la voix des mots isolés.
VOIX_PERSO = {
    "MANON":  "enseignante",
    "LEILA":  "feminin_2",
    "RACHID": "masculin_1",
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — le produire par "
              f"« node build/releve_sons.js module-n3-voisins > "
    ...
    print(f"🔊 module-n3-voisins — {len(taches)} extraits "
