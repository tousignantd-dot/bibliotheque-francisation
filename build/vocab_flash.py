...
.vf-planche { display: grid; grid-template-columns: minmax(0, 38%) minmax(0, 1fr); }
.vf-image { background: var(--surface-sunken); border-right: 1px solid var(--border);
...
.vf-image img { width: 100%; flex: 1; min-height: 0; height: 260px; object-fit: contain; display: block; padding: var(--sp-4); }
...
.vf-question { max-width: 760px; margin: 0 auto var(--sp-6); text-align: center;
  font-size: var(--fs-lead); font-weight: var(--fw-semi); line-height: var(--lh-loose); }
.vf-question img { width: min(560px, 100%); max-height: 300px; object-fit: contain;
...
@media (max-width: 1040px) {
  ...
  .vf-planche { grid-template-columns: 1fr; }
  .vf-image { border-right: none; border-bottom: 1px solid var(--border); }
}
