const PLUS = {
  ...
  prGraphie: {
    eye:'Mini-leçon', tit:"Les lettres qui ne disent pas ce qu'elles montrent",
    blocs:[
      {t:'texte', h:"Pourquoi ça se joue à l'hôpital plus qu'ailleurs",
       p:"Un mot de santé, vous l'entendez souvent avant de le lire. Quelqu'un vous dit « échographie » dans un corridor ; vous rentrez chez vous, vous l'écrivez comme vous l'avez entendu, et vous ne le retrouvez nulle part. Le mot existe pourtant : il ne s'écrit simplement pas comme il se dit. Trois cas seulement, et les mots de la santé en sont pleins parce qu'ils viennent presque tous du grec.",
      ...
      {t:'ana', h:"Premier cas — les lettres ch prononcées k",
       p:"C'est le plus utile des trois, parce que c'est celui des noms d'examens et de spécialités. Ces mots-là sont longs, savants, et ils voisinent souvent un y ou un ph.",
       mots:[['Ce que la page montre','une é{ch}ographie · {ch}ronique · un psy{ch}iatre · le {ch}olestérol'],
             ['Ce que la bouche fait','un k tout sec, comme au début de « carte »', true],
             ['Le repère qui ne trompe presque jamais','un mot de science ou de médecine : essayez le k avant le souffle de chat']],
       say:"une échographie, chronique, un psychiatre, le cholestérol",
       note:"Rassurez-vous : « chercher », « chaque », « chambre » n'ont rien à voir avec cette famille. Le k concerne une petite liste qu'on apprend une fois pour toutes."},

      {t:'ana', h:"Deuxième cas — la lettre x prononcée s",
       p:"Trois nombres, et pas un de plus. Mais ce sont trois nombres qu'une secrétaire vous dira quinze fois : un étage, un poste téléphonique, un nombre de semaines.",
       mots:[['Ce que la page montre','si{x} · di{x} · soi{x}ante-dix'],
             ['Ce que la bouche fait','un s franc, comme à la fin de « autobus »', true],
             ["Et la fin bouge selon ce qui suit",'seul, on entend le s ; devant une consonne, il disparaît ; devant une voyelle, il devient z']],
       say:"six, dix, soixante-dix",
       note:"Faites l'essai avec un délai : « dans six » se dit sisse, « six semaines » se dit si semaines, et « six ans » se dit siz ans. Le mot ne change pas, sa fin change."},

      {t:'ana', h:"Troisième cas — les lettres sh et sch prononcées ch",
       p:"Ces mots-là sont venus d'ailleurs et se sont installés. Ils sont courts, et rien dans leur écriture ne prévient.",
       mots:[['Ce que la page montre','un {sch}éma · un {sh}ampoing · un {sh}ort'],
             ['Ce que la bouche fait','le souffle de « chat », exactement', true],
             ['Où vous les rencontrerez ici','le schéma des étages sur un feuillet, le shampoing sur une liste de produits, le short demandé pour un examen à l\'effort']],
       say:"un schéma, un shampoing, un short",
       note:"Retenez surtout « un schéma » : c'est le dessin qui explique en un coup d'œil un parcours que trois paragraphes n'arrivaient pas à expliquer."},
      ...
      {t:'labo', h:"Le mot entendu, puis répété",
       ...
       out:{
         a1:{w:["une échographie"], say:"une échographie", n:'cinq syllabes, et la deuxième démarre sur un k'},
         a2:{w:["un psychiatre"], say:"un psychiatre", n:'psi-kiatre : deux consonnes muettes et un k au milieu'},
         b1:{w:["six"], say:"six", n:'isolé, le mot se termine par un s bien net'},
         b2:{w:["soixante-dix"], say:"soixante-dix", n:'deux x dans le même mot, deux fois le même s'},
       ...
       note:"Faites-le jouer deux fois avant d'ouvrir la bouche. Ce qu'on entraîne ici, c'est l'oreille ; la langue suit d'elle-même."},

      {t:'ex', h:"Huit mots, vus puis dits",
       p:"La colonne de gauche est ce que vous lirez ; celle de droite, ce que vous entendrez.",
       rows:[
         ["une échographie","é-co-gra-fie, avec un k et un f"],
         ["un psychiatre","psi-kiatre, comme psychologie"],
         ["le cholestérol","co-les-té-rol, sans aucun souffle de chat"],
         ["la technique","tec-nique, comme technologie"],
         ["six semaines","si semaines : le x disparaît devant la consonne"],
         ["dix heures","diz heures : devant la voyelle, il devient z"],
      ...
      {t:'piege', h:"Ce qui bloque, et ce qui n'est pas grave",
       rows:[
         ["écrire le mot exactement comme on l'a entendu","essayer la lettre muette",
          "Vous cherchez « écografie » et vous ne trouvez rien. Prenez l'habitude d'essayer un ch devant un k et un x devant un s : le mot apparaît presque toujours du premier coup."],
         ["donner à tous les ch le souffle de chat","apprendre la petite liste savante",
          "Dire « te-chnique » avec le souffle de « chat » rend le mot méconnaissable, et votre interlocuteur ne devinera pas. Ces mots se comptent sur les doigts."],
         ["se crisper sur les trois formes de six","viser la compréhension, pas la perfection",
          "Personne à l'accueil ne vous reprendra sur « siz semaines ». Ce qu'il faut, c'est comprendre le délai qu'on vous donne ; le produire parfaitement viendra plus tard."],
       ]},

      {t:'check', h:"Vérifions en quatre questions",
       p:"Une seule bonne réponse chaque fois.",
       qs:[
         {q:"Dans « échographie », les lettres ch produisent…", opts:["le souffle de chat","un k"], ok:1,
          fb:"Mot grec : é-co-gra-fie, avec un k bien net."},
         {q:"Dans « soixante-dix », la lettre x produit…", opts:["un s","le groupe ks"], ok:0,
          fb:"Soi-sante-dis. Le x y fait le même travail que dans six et dix."},
      ...
      {t:'revoir', h:"Ce qu'il faut garder",
       p:"Trois familles, et c'est tout : <b>ch</b> devient k dans les mots de science et de médecine (échographie, psychiatre, cholestérol) ; <b>x</b> devient s dans six, dix et soixante ; <b>sh</b> et <b>sch</b> prennent le souffle de chat dans les mots venus d'ailleurs (schéma, short). Devant un mot introuvable, essayez la lettre muette."},
    ]
  },
  ...
  t1pqp: {
    ...
    blocs:[
      ...
      {t:'ana', h:"Comment il se forme",
       ...
       mots:[["L'auxiliaire à l'imparfait",'j\'avais · tu avais · il avait · nous avions · vous aviez · ils avaient — ou j\'étais, il était…'],
      ...
      {t:'piege', h:"Deux confusions fréquentes",
       rows:[
         ...
         ["oublier l'accord avec être","refaire le réflexe du passé composé",
      ...
      {t:'check', h:"Vérifions en quatre questions",
       ...
       qs:[
         ...
         {q:"Le plus-que-parfait se forme avec…", opts:["l'auxiliaire à l'imparfait + le participe passé","l'auxiliaire au présent + l'infinitif"], ok:0,
  ...
  t3nom: {
    ...
    blocs:[
      ...
      {t:'ana', h:"Les terminaisons régulières",
  ...
  t3ps: {
    eye:'Mini-leçon', tit:"Un temps qu'on lit et qu'on n'écrit jamais",
    blocs:[
    ...
    blocs:[
      ...
      {t:'ana', h:"La virgule — celle qui encadre un ajout",
       ...
       mots:[["Ce que ça donne","Madame Demirci, 41 ans, a été vue ce jour."],
             ...
             ["Le test qui ne trompe pas","retirez ce qui est entre les deux virgules : si la phrase tient encore, c'était bien un ajout"]],
