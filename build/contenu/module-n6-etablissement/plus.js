const PLUS = {
  ...
  prGraphie: {
    ...
    blocs:[
      ...
      {t:'ana', h:"Premier cas — les lettres ch prononcées k",
       p:"Ces mots-là viennent presque tous du grec ancien. Ils désignent des sciences, et vous les rencontrerez sur une liste de matières bien avant de les entendre.",
       mots:[['Ce que la page montre','la psy{ch}ologie · une {ch}ronologie · un or{ch}estre · une {ch}orale'],
             ['Ce que la bouche fait','un k tout sec, comme au début de « carte »', true],
             ['Comment les repérer','ils sont longs, savants, et voisinent souvent un y ou un ph']],
       say:"la psychologie, une chronologie, un orchestre, une chorale",
       note:"Rassurez-vous tout de suite : « chercher », « chaque » et « chose » n'ont rien à voir avec cette famille. Le k concerne une petite liste, apprise une fois pour toutes."},

      {t:'ana', h:"Deuxième cas — la lettre x prononcée s",
       p:"Trois nombres, pas un de plus. Mais ce sont trois nombres qu'une secrétaire vous dira quinze fois : dates, délais, numéros de local.",
       mots:[['Ce que la page montre','si{x} · di{x} · soi{x}ante'],
             ['Ce que la bouche fait','un s franc, comme à la fin de « bus »', true],
             ["Ce qui change selon la suite",'seul, on entend le s ; devant une consonne, il disparaît ; devant une voyelle, il devient z']],
       say:"six, dix, soixante",
       note:"Faites l'essai avec une date : « le six » se dit sisse, « six février » se dit si février, et « six ans » se dit siz ans. Le mot est le même, la fin bouge."},

      {t:'ana', h:"Troisième cas — les lettres sh et sch prononcées ch",
       p:"Ces mots-là sont arrivés d'ailleurs et se sont installés. Ils sont courts, et rien dans leur écriture ne prévient.",
       mots:[['Ce que la page montre','un {sch}éma · un {sh}ampoing · un {sh}ort'],
             ['Ce que la bouche fait','le souffle de « chat », exactement', true],
             ['Comment les repérer','un mot bref, souvent un objet du quotidien']],
       say:"un schéma, un shampoing, un short",
       note:"Retenez surtout « un schéma » : une brochure de programme en contient presque toujours un, et c'est le dessin qui explique la démarche en un coup d'œil."},

      {t:'labo', h:"Le mot entendu, puis répété",
       p:"Prenez une famille, puis un exemple.",
       axes:[
         {id:'c', lbl:'Quelle famille ?', opts:[['a','ch prononcé k'],['b','x prononcé s'],['c','sh et sch prononcés ch']]},
         {id:'n', lbl:'Quel exemple ?', opts:[['1','le premier'],['2','le second']]}],
       out:{
         a1:{w:["la psychologie"], say:"la psychologie", n:'quatre syllabes, et la première fait psi'},
         a2:{w:["un orchestre"], say:"un orchestre", n:'la deuxième syllabe démarre sur un k'},
         b1:{w:["six"], say:"six", n:'isolé, le mot se termine par un s bien net'},
         b2:{w:["soixante"], say:"soixante", n:'aucun k au milieu, seulement un s'},
         c1:{w:["un schéma"], say:"un schéma", n:'trois consonnes écrites, un seul souffle'},
         c2:{w:["un short"], say:"un short", n:'le mot vient de loin, la bouche reste française'},
       },
       note:"Faites-le jouer deux fois avant d'ouvrir la bouche. Ce qu'on entraîne ici, c'est l'oreille ; la langue suivra d'elle-même."},

      {t:'ex', h:"Huit mots, vus puis dits",
       p:"La colonne de gauche est ce que vous lirez ; celle de droite, ce que vous entendrez.",
       rows:[
         ["la psychologie","psi-co-lo-gie, avec un k au milieu"],
         ["une chronologie","cro-no-lo-gie, sans aucun souffle de chat"],
         ["un orchestre","or-kestre, en deux temps"],
         ["la technologie","tec-no-lo-gie, comme technique"],
         ["six semaines","si semaines : le x disparaît devant la consonne"],
         ["soixante","soi-sante, jamais soi-ksante"],
         ["un schéma","ché-ma, trois lettres pour un souffle"],
         ["un short","chort, à la française"],
       ]},

      {t:'piege', h:"Ce qui bloque, et ce qui n'est pas grave",
       rows:[
         ["taper le mot tel qu'on l'a entendu","essayer la lettre muette",
          "Vous cherchez « cronologie » et le dictionnaire reste vide. Prenez l'habitude d'essayer un ch devant le k et un x devant le s : le mot apparaît presque toujours du premier coup."],
         ["donner à tous les ch le souffle de chat","apprendre la petite liste savante",
          "Dire « technologie » avec le souffle de « chat » rend le mot méconnaissable, et votre interlocuteur ne devinera pas. Ces mots se comptent sur les doigts : une carte suffit."],
         ["se crisper sur les trois formes de six","viser la reconnaissance, pas la perfection",
          "Aucune secrétaire ne vous reprendra sur « siz jours ». Ce qu'il faut, c'est comprendre la date qu'on vous donne — la produire parfaitement viendra plus tard, ou pas du tout."],
       ]},

      {t:'check', h:"Vérifions en quatre questions",
       p:"Une seule bonne réponse chaque fois.",
       qs:[
         {q:"Dans « psychologie », les lettres ch produisent…", opts:["le souffle de chat","un k"], ok:1,
          fb:"Mot grec : on dit psi-co-lo-gie, avec un k bien net."},
         {q:"Dans « soixante », la lettre x produit…", opts:["un s","le groupe ks"], ok:0,
          fb:"Soi-sante. Le x y fait exactement le même travail que dans six et dix."},
         {q:"Dans « un schéma », les lettres sch produisent…", opts:["le groupe sk","le souffle de chat"], ok:1,
          fb:"Trois consonnes à l'écrit, un seul souffle à l'oreille."},
         {q:"« Six semaines » se dit…", opts:["si semaines","sisse semaines"], ok:0,
          fb:"Devant une consonne, la fin de six s'efface complètement."},
       ]},

      {t:'revoir', h:"Ce qu'il faut garder",
       p:"Trois familles, et c'est tout : <b>ch</b> devient k dans les mots de science (psychologie, orchestre) ; <b>x</b> devient s dans six, dix et soixante ; <b>sh</b> et <b>sch</b> prennent le souffle de chat dans les mots venus d'ailleurs (short, schéma). Devant un mot introuvable, essayez la lettre muette."},
    ]
  },
  ...
  t1repr: {
    eye:'Mini-leçon', tit:"Trois mots de deux lettres, et le fil tient",
    blocs:[
      ...
      {t:'ana', h:"Le « le » qui tient lieu de toute une phrase",
       p:"Celui-là ne désigne aucun objet : il ramasse une phrase entière déjà prononcée.",
       mots:[['Ce qui a été dit','Je sais <u>que le test ne donne pas de diplôme</u>.'],
             ['Ce qui se dit ensuite','Je <b>le</b> sais.', true],
             ["Une invariabilité totale","aucun féminin, aucun pluriel : ce « le » ne bouge jamais"],
             ['Les verbes qui le réclament','savoir, dire, croire, ignorer, expliquer, répéter, comprendre']],
       say:"Je le sais. Il le dit. Elle l'a expliqué deux fois.",
       note:"C'est celui qui résiste le plus longtemps : l'oreille cherche une chose à quoi le rattacher, et il n'y en a aucune."},

      {t:'ana', h:"Le « en », qui ramasse un groupe commençant par de",
       p:"La préposition disparaît, et ce qui restait derrière le verbe passe devant lui.",
       mots:[['Ce qui a été dit','Il parle <u>des préalables particuliers</u>.'],
             ['Ce qui se dit ensuite','Il <b>en</b> parle.', true],
             ['Il sert aussi à compter',"J'ai <u>trois relevés</u> devient J'<b>en</b> ai trois."],
             ['Une frontière nette','Il parle <u>de sa conseillère</u> reste : Il parle <b>d\'elle</b>.']],
       say:"Il en parle. J'en ai trois. J'en ai besoin avant février.",
       note:"Faites une liste des verbes en « de » que vous employez au centre : parler de, avoir besoin de, s'occuper de, se souvenir de, dépendre de. Cinq suffisent."},

      {t:'ana', h:"Le « y », qui ramasse un à ou un endroit",
       p:"Le mécanisme est identique, avec la préposition « à » ou avec un lieu nommé.",
       mots:[['Une chose','Je pense <u>à ma demande</u> devient J\'<b>y</b> pense.'],
             ['Un endroit','Elle va <u>au secrétariat</u> devient Elle <b>y</b> va.', true],
             ['La même frontière','Je pense <u>à ma fille</u> reste : Je pense <b>à elle</b>.'],
             ["Un cas à part","dans « il <b>y</b> a », ce petit mot ne renvoie plus à rien"]],
       say:"J'y pense. Elle y va. On y trouve aussi les dates.",
       note:"Rosa dit exactement cela dans le dialogue du défi 2 : « on y trouve aussi les dates ». Le « y », c'est l'encadré gris de la page trois."},

      {t:'labo', h:"D'abord la version longue, ensuite la brève",
       p:"Prenez un petit mot, puis un exemple.",
       axes:[
         {id:'p', lbl:'Lequel ?', opts:[['a','le'],['b','en'],['c','y']]},
         {id:'n', lbl:'Quel exemple ?', opts:[['1','le premier'],['2','le second']]}],
       out:{
         a1:{w:["Je le sais."], say:"Je sais que le test ne donne pas de diplôme. Je le sais.", n:"toute la phrase soulignée tient dans deux lettres"},
         a2:{w:["Il l'a répété."], say:"Il a répété que ce n'est pas une équivalence. Il l'a répété.", n:"devant une voyelle, il perd sa voyelle à lui"},
         b1:{w:["Il en parle."], say:"Il parle des préalables particuliers. Il en parle.", n:"le groupe commençait par des : donc en"},
         b2:{w:["J'en ai besoin."], say:"J'ai besoin d'un relevé de notes. J'en ai besoin.", n:"avoir besoin de : la préposition commande"},
         c1:{w:["J'y pense."], say:"Je pense à ma demande d'admission. J'y pense.", n:"penser à quelque chose : donc y"},
         c2:{w:["Elle y va."], say:"Elle va au secrétariat du pavillon B. Elle y va.", n:"un endroit se ramasse aussi dans y"},
       },
       note:"Écoutez la longue, puis la brève, sans lire. C'est le trajet exact que votre oreille devra faire pendant un rendez-vous."},

      {t:'piege', h:"Trois habitudes à défaire",
       rows:[
         ["faire varier le « le » d'idée","le laisser tel quel",
          "Écrire « Elle la sait » suppose un mot féminin caché quelque part. Il n'y en a pas : ce « le » remplace une phrase, et une phrase n'a ni genre ni nombre. On écrit « Elle le sait »."],
         ["ramasser une personne dans en ou y","laisser la préposition debout",
          "« Je pense à ma fille » ne devient pas « j'y pense ». Une personne demande un pronom fort : je pense à elle, je parle de lui, je m'occupe d'eux. La règle vaut pour les deux petits mots."],
         ["ranger le pronom derrière le verbe","le poser devant",
          "« Je sais le » n'existe pas en français. Ces mots-là précèdent le verbe conjugué, et quand il y a deux verbes ils se collent à l'infinitif : « je vais en parler »."],
       ]},

      {t:'check', h:"Quatre questions pour vérifier",
       p:"Choisissez la bonne suite.",
       qs:[
         {q:"Dans « Il le savait », ce petit mot ramasse…", opts:["un objet précis","une phrase entière"], ok:1,
          fb:"Il ramasse tout ce qui avait été dit avant : une phrase, pas une chose."},
         {q:"« Il parle des préalables » se raccourcit en…", opts:["Il en parle","Il y parle"], ok:0,
          fb:"Le groupe commençait par « des » : c'est donc « en » qui le ramasse."},
         {q:"« Je pense à ma demande » se raccourcit en…", opts:["J'en pense","J'y pense"], ok:1,
          fb:"La préposition est « à » et il s'agit d'une chose : c'est « y »."},
         {q:"« Je pense à ma fille » se raccourcit en…", opts:["J'y pense","Je pense à elle"], ok:1,
          fb:"Devant une personne, la préposition reste et le pronom devient fort."},
       ]},

      {t:'revoir', h:"Ce qu'il faut garder",
       p:"Trois mots de deux lettres, trois questions à se poser. <b>le</b> ramasse une phrase déjà dite. <b>en</b> ramasse un groupe en « de ». <b>y</b> ramasse un groupe en « à » ou un endroit. Dès que vous en entendez un, remontez d'une phrase : ce qu'il remplace est juste au-dessus."},
    ]
  },
  ...
  t1celui: {
    ...
    blocs:[
      ...
      {t:'check', h:"Trois questions pour vérifier",
       p:"Une seule bonne réponse par question.",
       qs:[
  ...
  t2avis: {
    ...
    blocs:[
      ...
      {t:'check', h:"Trois questions pour vérifier",
       p:"À faire de tête, sans revenir en arrière.",
       qs:[
  ...
  t2passif: {
    ...
    blocs:[
      ...
      {t:'check', h:"Trois questions pour vérifier",
       p:"Répondez avant de relire le tableau.",
       qs:[
  ...
  t3subj: {
    ...
    blocs:[
      ...
      {t:'check', h:"Quatre questions, pour être sûr",
       p:"Prenez le temps de relire chaque phrase.",
       qs:[
  ...
  t3si: {
    ...
    blocs:[
      ...
      {t:'piege', h:"Deux pièges",
       rows:[
         ["glisser un futur juste après si","garder le présent",
