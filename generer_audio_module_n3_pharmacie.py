#!/usr/bin/env python3
"""
Générateur d'audio — module « Aller à la pharmacie » (module-n3-pharmacie, niveau 3).
...
Les dialogues sont lus dans `build/contenu/module-n3-pharmacie/dialogues.js` :
une seule source, comme aux niveaux 2 et 5.

Les identifiants de la famille 2 ne s'inventent pas — ils sont produits par le
moteur au rendu. `sons_module_n3_pharmacie.json` a été produit hors navigateur
par `node build/releve_sons.js module-n3-pharmacie`, qui rejoue sur les fichiers de
...
Usage :  python3 generer_audio_module_n3_pharmacie.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n3-pharmacie"
MANIFESTE = RACINE / "sons_module_n3_pharmacie.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n3-pharmacie/dialogues.js"
...
# moins un dialogue, donc aucune ne peut être partagée. Gaétane, l'assistante
# technique, explique le renouvellement à Nadia : elle prend la voix
# « enseignante », ralentie à 0,85 par voix_lente.py.
VOIX_PERSO = {
    "NADIA":   "feminin_2",
    "GAÉTANE": "enseignante",
    "ÉTIENNE": "masculin_1",
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"relancer node node build/releve_sons.js module-n3-pharmacie")
        sys.exit(1)
    ...
    print(f"🔊 module-n3-pharmacie — {len(taches)} extraits "
