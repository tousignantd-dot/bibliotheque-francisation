#!/usr/bin/env python3
"""
Générateur d'audio — module « Choisir une activité au centre »
(module-n3-loisirs, niveau 3, activité 85).
...
**321 extraits attendus** : 87 répliques sur six dialogues, et 234 sons.

Les dialogues sont lus dans `build/contenu/module-n3-loisirs/dialogues.js` :
une seule source, comme aux niveaux 2, 3 et 5.

Les identifiants de la famille 2 ne s'inventent pas — ils sont produits par le
moteur au rendu. `sons_module_n3_loisirs.json` a été produit **hors
navigateur**, par :

    node build/releve_sons.js module-n3-loisirs > sons_module_n3_loisirs.json

Vingt lignes de node qui rejouent sur les fichiers de contenu les trois
endroits du gabarit appelant `playWord` : le bloc `savoir`
(`ex.id+'_savoir_'+ri+'_'+wi`, avec la phrase porteuse de `CARRIER_PHRASES`),
les exercices `vf` à cartes (`ex.id+'_'+r.id`) et `plAudioManifest()` pour les
mini-leçons.

**`build/collecte_sons.py` n'a pas à être lancé du tout** — et ne doit pas
l'être : il n'expire pas, il attend un seul envoi, et il a déjà écrasé un
relevé complet par un relevé partiel longtemps après qu'on avait cessé d'y
penser. Le relevé hors navigateur rend exactement les mêmes clés.

**Les clés de `CARRIER_PHRASES` sont les mots accentués** tels qu'ils
paraissent dans les listes `savoir[…][2]` : le gabarit fait
`CARRIER_PHRASES[w]` sur le mot affiché, et une clé écrite en slug ne serait
jamais trouvée — la pastille lirait alors le mot seul, mal prononcé. Les 82
clés de ce module ont été relevées par script, dans les deux sens (zéro
manquante, zéro inutilisée), et `node build/coherence.js module-n3-loisirs`
le revérifie.

**État au 22 août 2026 : aucun extrait n'a encore été produit.** La session
qui a écrit le module n'avait pas les clés d'API. Le module est livré muet,
et le son se rattrape d'un seul coup :

    python3 generer_audio_module_n3_loisirs.py
...
Usage :  python3 generer_audio_module_n3_loisirs.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n3-loisirs"
MANIFESTE = RACINE / "sons_module_n3_loisirs.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n3-loisirs/dialogues.js"
...
# Quatre personnages, quatre voix distinctes. Marisol garde la voix de
# l'enseignante — c'est elle qui porte le module, et c'est aussi la voix des
# mots isolés et des mini-leçons. Thierry et Denis ne se répondent jamais
# dans un même dialogue, mais il reste une voix libre : autant la leur
# donner, plutôt que d'obliger l'élève à distinguer deux hommes au timbre
# identique d'un défi à l'autre.
VOIX_PERSO = {
    "MARISOL": "enseignante",
    "ROXANE":  "feminin_2",
    "THIERRY": "masculin_1",
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — le produire par "
              f"« node build/releve_sons.js module-n3-loisirs > "
    ...
    print(f"🔊 module-n3-loisirs — {len(taches)} extraits "
