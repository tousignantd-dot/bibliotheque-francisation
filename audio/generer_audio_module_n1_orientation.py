...
Générateur d'audio — module « Je lis les panneaux du centre » (niveau 1)
(module-n1-orientation, activité 97).
...
`build/contenu/module-n1-orientation/dialogues.js`, la source unique. Une copie
locale divergerait du module à la première correction.

Les identifiants de la famille 2 ne s'inventent pas non plus — ils viennent du
moteur. **`build/collecte_sons.py` n'a pas à être lancé** : le relevé se fait
hors navigateur, en une commande, et rend les mêmes clés à l'octet près (voir
CLAUDE.md, où deux incidents de collecteur laissé en tâche de fond sont
racontés) :

    node build/releve_sons.js module-n1-orientation > sons_module_n1_orientation.json

Les extraits courts passent par `charge_utile()` de `build/voix.py` : le modèle
reçoit une phrase française avant et après le mot, qui conditionne la synthèse
sans être prononcée. Sans elle, un mot nu comme « tirez » ou « la » ressort
régulièrement à l'anglaise — et ce module en compte beaucoup, puisqu'il fait
lire des mots de panneau, seuls, par définition.

Usage :  python3 generer_audio_module_n1_orientation.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n1-orientation"
MANIFESTE = RACINE / "sons_module_n1_orientation.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n1-orientation/dialogues.js"
...
# Trois personnages pour trois voix. Madame Paré, qui tient l'accueil, prend
# la voix de l'enseignante — celle que `voix_lente` ralentit à 0,85 : c'est
# elle qui explique, et au niveau 1 la vitesse est le premier obstacle. Rosa et
# Kofi se répondent dans le même dialogue ; ils ne peuvent pas partager une
# voix, et Rosa et madame Paré non plus.
VOIX_PERSO = {
    "ROSA":        "feminin_2",
    "KOFI":        "masculin_1",
    "MADAME PARÉ": "enseignante",
}
...
def main():
    ...
    print(f"🔊 module-n1-orientation — {len(taches)} extraits "
