#!/usr/bin/env python3
"""
Générateur d'audio — module « Je me présente » (niveau 1) (module-n1-presenter).
...
`build/contenu/module-n1-presenter/dialogues.js`, la source unique.
...
`build/collecte_sons.py`, qui écrit `sons_module_n1_presenter.json`.

Usage :  python3 generer_audio_module_n1_presenter.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n1-presenter"
MANIFESTE = RACINE / "sons_module_n1_presenter.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n1-presenter/dialogues.js"
...
# Quatre personnages pour quatre voix. Monsieur Tremblay, au secrétariat,
# prend la voix du narrateur : il n'apparaît qu'au dernier dialogue, où Paul
# n'est pas.
VOIX_PERSO = {
    "AMINA":             "feminin_2",
    "MADAME ROY":        "enseignante",
    "LIN":               "enseignante",
    "PAUL":              "masculin_1",
    "MONSIEUR TREMBLAY": "narrateur",
}
...
def main():
    ...
    for dial_id, lignes in dialogues.items():
        for i, (perso, texte) in enumerate(lignes, 1):
            ...
            # synthèse sans être prononcé. Voir l'en-tête pour
            # l'épellation.
    ...
    print(f"🔊 module-n1-presenter — {len(taches)} extraits "
