#!/usr/bin/env python3
"""
Générateur d'audio — module « Trouver son chemin » (module-deplacement).
...
`build/contenu/module-deplacement/dialogues.js`, la source unique.
...
`build/collecte_sons.py`, qui écrit `sons_module_deplacement.json`.

Usage :  python3 generer_audio_module_deplacement.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-deplacement"
MANIFESTE = RACINE / "sons_module_deplacement.json"
DIALOGUES_JS = RACINE / "build/contenu/module-deplacement/dialogues.js"
...
# Cinq personnages pour quatre voix. Gilles et Patrice partagent la voix
# masculine #1 : ils ne se répondent jamais — Gilles n'apparaît qu'au terminus,
# Patrice qu'au centre de formation. La voix du narrateur est réservée aux
# annonces diffusées, qui ne sont adressées à personne en particulier.
VOIX_PERSO = {
    "NOUR":        "feminin_2",
    "MARIE-JOSÉE": "enseignante",
    "GILLES":      "masculin_1",
    "PATRICE":     "masculin_1",
    "VOIX":        "narrateur",
}
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"lancer build/collecte_sons.py module-deplacement")
        sys.exit(1)
    ...
    print(f"🔊 module-deplacement — {len(taches)} extraits "
