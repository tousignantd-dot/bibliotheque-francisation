...
Générateur d'audio — module « Ça bloque ce matin »
(module-n5-transport, niveau 5, activité 69).
...
Les dialogues sont lus dans `build/contenu/module-n5-transport/dialogues.js` :
une seule source.

`sons_module_n5_transport.json` a été relevé **hors navigateur**, par
`node build/releve_sons.js module-n5-transport`. 200 extraits de la famille 2,
plus 57 répliques.
...
- **`build/collecte_sons.py` n'a pas été lancé, et ne doit pas l'être.** Il
  ...
  sans ouvrir de port.
- **Tout bloc `ana` porte son champ `say:`.** Les quarante blocs `ana` de ce
  module ont été vérifiés sur le relevé : aucune valeur ne contient de balise
  HTML, ce qui est la signature d'un `say:` manquant.
- **Les clés de `CARRIER_PHRASES` sont les mots accentués**, tels qu'ils
  paraissent dans les listes `savoir[…][2]`. Deux des seize mots portent une
  apostrophe — « l'accotement » et « un véhicule d'urgence » — et leurs clés
  sont écrites entre guillemets doubles, à l'identique de fccards.js.

Une particularité de ce module : les dix cartes de l'exercice `prPhon` sont
des mots ou des groupes courts dits **sans** phrase porteuse. C'est voulu, et
c'est le seul endroit du projet où ce l'est : l'exercice demande d'entendre la
voyelle nasale d'un mot — « un pont », « un bouchon », « un ralentissement » —
et une phrase porteuse mettrait justement l'oreille sur autre chose. Les
articles suffisent à donner l'accentuation.
...
Usage :  python3 generer_audio_module_n5_transport.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n5-transport"
MANIFESTE = RACINE / "sons_module_n5_transport.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n5-transport/dialogues.js"
...
# Quatre locuteurs, quatre voix. La règle est tenue : deux personnages ne
# partagent une voix que s'ils ne se répondent jamais dans un même dialogue —
# ici les quatre voix sont distinctes, donc la question ne se pose pas.
#
# TEREZA traverse les quatre dialogues : voix féminine #2. AMINE, le collègue
# de covoiturage, est dans trois d'entre eux : voix masculine #1. GAÉTAN est
# l'animateur de la chronique de circulation : voix de narrateur, la seule qui
# tienne le débit rapide et régulier d'un bulletin. GHISLAINE, la responsable
# de l'atelier, prend la voix « enseignante », ralentie à 0,85 par
# voix_lente.py — c'est elle qui pose au téléphone les questions dont l'élève
# doit tirer sa réponse, et un débit plus lent est exactement ce qu'il faut.
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"lancer node build/releve_sons.js module-n5-transport")
        sys.exit(1)
    ...
    print(f"🔊 module-n5-transport — {len(taches)} extraits "
