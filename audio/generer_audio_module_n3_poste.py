...
Générateur d'audio — module « Le colis de Yassine » (module-n3-poste, niveau 3).
...
**Rien n'a encore été généré au 21 août 2026 : le compte ElevenLabs était à
zéro crédit** (401 `quota_exceeded`, vérifié sur un extrait de deux mots avant
d'engager quoi que ce soit). Le module est complet et livré sans son ; les
pastilles resteront muettes tant que ce script n'aura pas tourné. Une fois le
compte rechargé, une seule commande, sans autre préparation :

    python3 generer_audio_module_n3_poste.py

316 extraits attendus — 72 répliques sur 5 dialogues, plus les 244 clés de
`sons_module_n3_poste.json`. Le script est relançable : un fichier déjà
présent est sauté, donc une coupure en cours de route ne coûte rien.

Les dialogues sont lus dans `build/contenu/module-n3-poste/dialogues.js` :
une seule source, comme aux niveaux 2, 3 et 5.

Les identifiants de la famille 2 ne s'inventent pas — ils sont produits par le
moteur au rendu. `sons_module_n3_poste.json` vient de :

    node build/releve_sons.js module-n3-poste > sons_module_n3_poste.json

qui rejoue sur les fichiers de contenu les trois endroits du gabarit appelant
`playWord` : le bloc `savoir` (`ex.id+'_savoir_'+ri+'_'+wi`, avec la phrase
porteuse de `CARRIER_PHRASES`), les exercices `vf` à cartes (`ex.id+'_'+r.id`)
et `plAudioManifest()` pour les mini-leçons.
...
jamais trouvée. Relevé fait dans les deux sens ici : 43 mots, 43 clés, aucune
en trop.
...
programme. Un 401 ne se réessaie pas — c'est le quota, pas le réseau.

Usage :  python3 generer_audio_module_n3_poste.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n3-poste"
MANIFESTE = RACINE / "sons_module_n3_poste.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n3-poste/dialogues.js"
...
# Trois personnages. Denise, la voisine qui explique le bureau de poste et
# qui prend des nouvelles à la fin, reçoit la voix « enseignante », ralentie à
# 0,85 par voix_lente.py : c'est elle qui pose le vocabulaire. Carole, la
# préposée au comptoir, garde une voix à vitesse normale — le dialogue `prep`
# dit justement qu'elle « parle un peu vite », et la ralentir démentirait
# l'exercice d'écoute qui suit. Denise et Carole ne se répondent jamais dans
# un même dialogue, mais elles ont tout de même deux voix distinctes.
VOIX_PERSO = {
    "YASSINE": "masculin_1",
    "DENISE":  "enseignante",
    "CAROLE":  "feminin_2",
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"relancer node build/releve_sons.js module-n3-poste")
        sys.exit(1)
    ...
    print(f"🔊 module-n3-poste — {len(taches)} extraits "
