#!/usr/bin/env python3
"""
Générateur d'audio — module « Acheter des vêtements » (module-vetements).
...
`build/contenu/module-vetements/dialogues.js`, la source unique.
...
`build/collecte_sons.py`, qui écrit `sons_module_vetements.json`.

Usage :  python3 generer_audio_module_vetements.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
MANIFESTE = RACINE / "sons_module_vetements.json"
DIALOGUES_JS = RACINE / "build/contenu/module-vetements/dialogues.js"
...
# Quatre personnages pour quatre voix. Rémi, au service à la clientèle, prend
# la voix du narrateur : il n'apparaît qu'au comptoir des échanges, où Valérie
# n'est pas.
VOIX_PERSO = {
    "KOFI":    "masculin_1",
    "VALÉRIE": "enseignante",
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"lancer build/collecte_sons.py module-vetements")
        sys.exit(1)
    ...
    print(f"🔊 module-vetements — {len(taches)} extraits "
