"""
Générateur d'audio — module « Rester locataire ou devenir propriétaire »
(module-n7-logement, niveau 7, activité 111).
...
**294 extraits attendus** : 92 répliques sur quatre dialogues, et 202 sons.

Les dialogues sont lus dans `build/contenu/module-n7-logement/dialogues.js` :
...
moteur au rendu. `sons_module_n7_logement.json` a été produit **hors
navigateur**, par :

    node build/releve_sons.js module-n7-logement > sons_module_n7_logement.json
...
**Quatre personnages, trois timbres** :

    prep  SOKHNA, GÉRALD        t1  SOKHNA, GÉRALD
    t2    SOKHNA, JOSIANE       t3  SOKHNA, FARAH

JOSIANE (la courtière, défi 2) et FARAH (la conseillère hypothécaire, défi 3)
...
justement ces deux dialogues-là : ce sont eux qui énoncent les délais, les
pourcentages et les montants que l'élève devra redire dans « Je me lance ».
SOKHNA et GÉRALD gardent un débit normal — au niveau 7, la compétence vise le
débit normal.

**État au 22 août 2026 : aucun extrait n'a encore été produit.** Le compte
...
muet, et le son se rattrape d'un seul coup :
    ...
    python3 generer_audio_module_n7_logement.py
...
Usage :  python3 generer_audio_module_n7_logement.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n7-logement"
MANIFESTE = RACINE / "sons_module_n7_logement.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n7-logement/dialogues.js"
...
# Quatre personnages, trois timbres. JOSIANE (défi 2) et FARAH (défi 3)
...
# par `voix_lente.py` — ce sont elles qui énoncent les délais et les montants.
VOIX_PERSO = {
    "SOKHNA":  "feminin_2",
    "GÉRALD":  "masculin_1",
    "JOSIANE": "enseignante",
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — le produire par "
              f"« node build/releve_sons.js module-n7-logement > "
    ...
    print(f"🔊 module-n7-logement — {len(taches)} extraits "
