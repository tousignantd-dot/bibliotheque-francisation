#!/usr/bin/env python3
"""
Générateur d'audio — module « Ce que j'en pense, et pourquoi » (niveau 7)
(module-n7-oeuvres).
...
`build/contenu/module-n7-oeuvres/dialogues.js`, la source unique.

Niveau 7, donc des **discours longs** : les quatre extraits font de vingt-trois
à vingt-cinq répliques, quatre-vingt-dix-sept en tout. Le Défi 1 s'ouvre sur
six répliques d'affilée d'un même locuteur — l'extrait du spectacle d'humour,
qui ne s'adresse à personne dans la scène. Il prend la voix « narrateur », qui
n'est **pas** ralentie par `voix_lente.py` : un humoriste au débit posé n'est
plus un humoriste, et l'exercice porte justement sur le ton.

Le relevé des sons (famille 2) n'a **pas** été fait par le navigateur ni par
`build/collecte_sons.py`, qu'il ne faut pas lancer :

    node build/releve_sons.js module-n7-oeuvres > sons_module_n7_oeuvres.json
...
Usage :  python3 generer_audio_module_n7_oeuvres.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n7-oeuvres"
MANIFESTE = RACINE / "sons_module_n7_oeuvres.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n7-oeuvres/dialogues.js"
...
# ne se répondent jamais dans un même extrait — et cela s'est compté AVANT
# d'écrire les dialogues, pas après, ce qui est tout l'objet de la note laissée
# par `module-n7-publicite` et `module-n7-habitation` :
#
#   prep  MARILOU, GHYSLAINE           t1  RÉJEAN, MARILOU, GAÉTAN
#   t2    LUDOVIC, NADIA, MARILOU      t3  GHYSLAINE, MARILOU, GAÉTAN
#
# GHYSLAINE et NADIA partagent `enseignante` sans jamais se croiser ; RÉJEAN
# (l'humoriste, entendu en extrait au Défi 1) et LUDOVIC (l'animateur de radio
# du Défi 2) partagent `narrateur`, eux non plus. Aucun extrait ne réunit plus
# de trois locuteurs, ni plus de deux personnes du même genre.
#
# Le choix de qui prend `enseignante` n'est pas neutre : c'est la voix que
# `voix_lente` ralentit à 0,85. Elle va à Ghyslaine, qui préside le comité et
# explique posément à Marilou ce qu'on attend d'elle, et à Nadia Ferron, qui
# décortique sa propre chanson en entrevue — deux rôles d'explication, le
# débit posé leur va.
#
# RÉJEAN prend `narrateur`, et c'est délibéré : ses six répliques d'ouverture
# sont un extrait de spectacle, elles ne s'adressent à personne dans la scène,
# et cette voix n'est pas ralentie. Un humoriste dont on ralentit le débit
# perd ses silences, et le sketch tient sur ses silences.
VOIX_PERSO = {
    "MARILOU":    "feminin_2",
    "GHYSLAINE":  "enseignante",
    "GAÉTAN":     "masculin_1",
    "RÉJEAN":     "narrateur",
    "LUDOVIC":    "narrateur",
    "NADIA":      "enseignante",
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"lancer build/collecte_sons.py module-n7-oeuvres")
        sys.exit(1)
    ...
    print(f"🔊 module-n7-oeuvres — {len(taches)} extraits "
