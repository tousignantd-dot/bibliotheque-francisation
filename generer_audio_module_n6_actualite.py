"""
Générateur d'audio — module « Suivre un sujet dans les médias »
(module-n6-actualite, niveau 6, activité 99, module pilote du niveau).
...
**232 extraits attendus** : 75 répliques sur quatre dialogues, et 157 sons.

Les dialogues sont lus dans `build/contenu/module-n6-actualite/dialogues.js` :
une seule source, comme aux niveaux 2, 3 et 5.

Les identifiants de la famille 2 ne s'inventent pas — ils sont produits par le
moteur au rendu. `sons_module_n6_actualite.json` a été produit **hors
navigateur**, par :

    node build/releve_sons.js module-n6-actualite > sons_module_n6_actualite.json
...
Les extraits sont longs : dix-huit à vingt répliques par dialogue, dont
plusieurs de quarante mots — une chronique de radio et une entrevue ne se
découpent pas en saynètes de trois répliques. Le coût par extrait est donc
plus élevé qu'aux niveaux 2 et 3, à nombre d'extraits égal.
...
plein de mots courts que l'anglais connaît aussi — « six », « dix », « short »,
« documentaire » — et l'exercice de graphie-phonie les envoie **seuls** à la
synthèse : le relevé rend `prGraphie_gr12 → un short`, pas la phrase porteuse.
Sans contexte français, « un short » sort à l'anglaise, et c'est justement le
mot dont l'élève doit entendre la prononciation française. `charge_utile()` pose
`previous_text` et `next_text` autour de tout extrait de quatre mots ou moins ;
rien de ce contexte n'est prononcé ni facturé.
...
secondes. Les clés des mots de l'exercice sont donc **inutilisées par le
moteur** et gardées pour mémoire ; `node build/coherence.js` ne les compte pas
comme un écart, à raison.

**Les six voix pour quatre timbres** — deux ne se croisent jamais dans un même
extrait, c'est vérifié :

    prep  NADÈGE, RAPHAËL           t1  THÉO, CLAUDINE
    t2    THÉO, MYRIAM, NARRATRICE  t3  RAPHAËL, NADÈGE

NADÈGE, CLAUDINE et MYRIAM partagent `feminin_2` sans jamais se répondre. Seule
NARRATRICE prend la voix « enseignante », ralentie à 0,85 par `voix_lente.py` :
c'est la voix hors champ du documentaire, et un débit posé y est un avantage,
non un défaut. Les personnages de radio, eux, gardent un débit normal — au
...
muet, et le son se rattrape d'un seul coup :
    ...
    python3 generer_audio_module_n6_actualite.py
...
Usage :  python3 generer_audio_module_n6_actualite.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n6-actualite"
MANIFESTE = RACINE / "sons_module_n6_actualite.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n6-actualite/dialogues.js"
...
# seulement s'ils ne se répondent jamais dans un même extrait ; c'est vérifié
# dans le docstring. Seule NARRATRICE, la voix hors champ du documentaire,
# prend celle de l'enseignante — et son ralentissement à 0,85 y est un
# avantage. Les voix de radio gardent un débit normal : au niveau 6, la
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — le produire par "
              f"« node build/releve_sons.js module-n6-actualite > "
    ...
    print(f"🔊 module-n6-actualite — {len(taches)} extraits "
