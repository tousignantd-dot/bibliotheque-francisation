#!/usr/bin/env python3
"""
Générateur d'audio — module « Au restaurant » (module-restaurant).
...
`build/contenu/module-restaurant/dialogues.js`, la source unique.
...
`build/collecte_sons.py`, qui écrit `sons_module_restaurant.json`.

Usage :  python3 generer_audio_module_restaurant.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-restaurant"
MANIFESTE = RACINE / "sons_module_restaurant.json"
DIALOGUES_JS = RACINE / "build/contenu/module-restaurant/dialogues.js"
...
# Quatre personnages pour quatre voix. Vincent, le serveur en salle, prend la
# voix du narrateur ; Sophie, à l'accueil, la voix féminine #2 — elles ne se
# répondent jamais, Sophie n'apparaissant qu'à l'entrée et au dialogue du midi.
VOIX_PERSO = {
    "ANDRÉS":  "masculin_1",
    "MANON":   "enseignante",
    "VINCENT": "narrateur",
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"lancer build/collecte_sons.py module-restaurant")
        sys.exit(1)
    ...
    print(f"🔊 module-restaurant — {len(taches)} extraits "
