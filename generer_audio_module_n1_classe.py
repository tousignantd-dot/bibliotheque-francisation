#!/usr/bin/env python3
"""
Générateur d'audio — module « Regardez le tableau » (niveau 1) (module-n1-classe).
...
`build/contenu/module-n1-classe/dialogues.js`, la source unique. Les
générateurs des premiers modules en gardaient une copie, qui divergeait du
module à la première correction.

Les identifiants de la famille 2 ne s'inventent pas non plus : ils viennent du
moteur. Ils ont été relevés **hors navigateur** —

    node build/releve_sons.js module-n1-classe > sons_module_n1_classe.json

— et `build/collecte_sons.py` n'a pas été lancé une seule fois. Le relevé par
le navigateur demande un port, un collecteur en tâche de fond et un envoi qui
peut arriver longtemps après qu'on a cessé d'y penser ; il a déjà écrasé un
relevé complet par un relevé partiel, une fois les MP3 payés.

**Le contexte français des extraits courts passe par `build/voix.py`.**
`charge_utile()` ajoute `previous_text` / `next_text` — du texte qui
conditionne la synthèse sans être prononcé. Sur un mot isolé, c'est une phrase
de classe générique, posée dès que l'extrait fait quatre mots ou moins ; sur
une réplique de dialogue, ce sont **les répliques voisines de la scène**, qui
valent mieux, et qui partent quelle que soit la longueur du texte. Ce
module en est plein : c'est le niveau 1, les pastilles disent « un livre »,
« midi », « deux », « douze ». Sans contexte, `eleven_multilingual_v2` devine
la langue sur trois syllabes et rend régulièrement un mot à l'anglaise. Rien de
ce contexte n'est facturé : seul `text` est synthétisé.

Usage :  python3 generer_audio_module_n1_classe.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n1-classe"
MANIFESTE = RACINE / "sons_module_n1_classe.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n1-classe/dialogues.js"
...
# Trois personnages, trois voix. Madame Cyr prend celle de l'enseignante, que
# `voix_lente` ralentit à 0,85 : elle donne toutes les consignes du module, et
# une consigne mal entendue fait rater l'exercice entier. Bopha et Ivan se
# répondent dans le même dialogue, ils ne peuvent pas partager une voix.
VOIX_PERSO = {
    "BOPHA":      "feminin_2",
    "IVAN":       "masculin_1",
    "MADAME CYR": "enseignante",
}
...
def slug(nom):
    ...
    bien un `line_03_jean-philippe.mp3`. Ici, « MADAME CYR » donne
    `madame_cyr`.
    """
...
ATTENTE_BASE_S = 2   # doublée à chaque échec : 2, 4, 8, 16 s


def parle(cle, texte, voix, chemin, avant=None, apres=None):
    """Un extrait, avec reprise sur coupure réseau.

    Une coupure TLS en plein téléversement (`SSLEOFError`) n'est pas un refus
    de débit : la liaison revient en deux secondes. La base d'attente est donc
    courte — attendre 4, 8, 16 puis 32 secondes pour une panne passagère
    faisait perdre plus de temps que la panne elle-même.
    """
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"node build/releve_sons.js module-n1-classe > {MANIFESTE.name}")
        sys.exit(1)
    ...
    print(f"🔊 module-n1-classe — {len(taches)} extraits "
