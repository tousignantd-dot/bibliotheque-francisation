"""
Générateur d'audio — module « Entrer dans le programme »
(module-n7-etablissement, niveau 7, activité 118).
...
moteur au rendu. `sons_module_n7_etablissement.json` a été produit **hors
navigateur**, par :

    node build/releve_sons.js module-n7-etablissement > sons_module_n7_etablissement.json
...
ou moins ne sorte pas à l'anglaise ou à l'espagnole. Ce module en a un besoin
particulier : son exercice de phonétique fait entendre des mots nus — « tenir »,
« demander », « petite », « samedi » — dont l'enjeu est justement le « e »
français.

**Cinq personnages, quatre timbres** :

    prep  RANIA, GHYSLAINE          t1  RANIA, ÉMILIEN
    t2    RANIA, ÉMILIEN, YVAN      t3  RANIA, NADINE, ÉMILIEN

**État au 23 août 2026 : aucun extrait n'a encore été produit.** Le mandat de
la vague 7 disait d'écrire le générateur sans le lancer : les générateurs de
la vague partent ensemble par `build/audio_tous.py`. Le module est donc livré
muet, et le son se rattrape d'un seul coup :

    python3 generer_audio_module_n7_etablissement.py
...
Usage :  python3 generer_audio_module_n7_etablissement.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n7-etablissement"
MANIFESTE = RACINE / "sons_module_n7_etablissement.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n7-etablissement/dialogues.js"
...
# Cinq personnages, quatre timbres. Les locuteurs ont été comptés **par
# extrait avant l\'écriture des dialogues** : aucun extrait ne réunit trois
# personnes du même genre, contrainte payée par l\'activité 115 et qui se
# prévoit gratuitement. GHYSLAINE (la collègue, Je découvre) et NADINE (le
# secrétariat, défi 3) partagent « enseignante » : elles ne paraissent jamais
# dans le même dialogue et ne se répondent jamais. C\'est la seule mise en
# commun. La voix « enseignante » est ralentie à 0,85 par `voix_lente.py`,
# ce qui sert la collègue qui explique le contingentement et la technicienne
# qui répète ce qu\'elle n\'a pas le droit de dire. RANIA, ÉMILIEN et YVAN
# gardent un débit normal : au niveau 7, la compétence vise le débit normal,
# et l\'entrevue de sélection est justement l\'épreuve du débit normal.
VOIX_PERSO = {
    "RANIA":     "feminin_2",
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — le produire par "
              f"« node build/releve_sons.js module-n7-etablissement > "
    ...
    print(f"🔊 module-n7-etablissement — {len(taches)} extraits "
