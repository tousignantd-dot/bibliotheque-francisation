#!/usr/bin/env python3
"""
Générateur d'audio — module « Présenter un projet au travail » (niveau 7)
(module-n7-emploi, activité 109).
...
`build/contenu/module-n7-emploi/dialogues.js`, la source unique.

Ce module est lourd en audio, et c'est voulu. Au niveau 7 la compétence vise
des **discours longs** : la présentation de Renaud au Défi 1 et celle d'Aïcha
au Défi 2 sont des monologues coupés de questions, pas des saynètes — plusieurs
répliques y font quatre ou cinq phrases. C'est ce qui permet les écoutes
successives, une première pour le plan, une deuxième pour les chiffres, une
troisième pour le détail.

Une particularité de ce module, à l'exercice `prProso` : le bandeau ne fait pas
écouter des mots isolés mais des **groupes rythmiques**, et leurs phrases
porteuses sont les phrases entières d'où ils sont tirés. C'est dans la phrase
que s'entend la montée de la voix ; un groupe dit seul descendrait toujours.

Les identifiants de la famille 2 ne s'inventent pas : ils sont relevés hors
navigateur par `node build/releve_sons.js module-n7-emploi >
sons_module_n7_emploi.json`. Ne pas lancer `build/collecte_sons.py`.

Usage :  python3 generer_audio_module_n7_emploi.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n7-emploi"
MANIFESTE = RACINE / "sons_module_n7_emploi.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n7-emploi/dialogues.js"
...
# Quatre personnages, quatre voix : aucun partage, donc aucun croisement à
# vérifier.
#
...
# `voix_lente` ralentit à 0,85. Elle va à Thérèse Lapointe, la représentante
# en santé et en sécurité — le personnage qui explique, à qui un débit posé
# convient. Aïcha garde un débit normal : c'est elle qui porte la présentation
# du Défi 2, et cette présentation est l'exercice d'écoute long du module.
# Renaud, dont la présentation du Défi 1 est l'autre écoute longue, garde lui
# aussi son débit : le ralentir retirerait la difficulté que l'exercice existe
# pour créer.
VOIX_PERSO = {
    "AÏCHA":   "feminin_2",
    "THÉRÈSE": "enseignante",
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"lancer node build/releve_sons.js module-n7-emploi "
              f"> {MANIFESTE.name}")
        sys.exit(1)
    ...
    print(f"🔊 module-n7-emploi — {len(taches)} extraits "
