...
Générateur d'audio — module « Chercher un emploi » (niveau 6) (module-n6-recherche).
...
Copié de `generer_audio_module_n2_autobus.py`, dont il garde la reprise
réseau : ElevenLabs coupe par intermittence, et un échec réseau ne veut pas
dire que l'extrait est en cause — relancer plus tard suffit.

Les dialogues sont lus dans `build/contenu/module-n6-recherche/dialogues.js`,
source unique. Les identifiants de la famille 2 viennent du moteur, au rendu,
et sont reçus par `build/collecte_sons.py`, qui écrit
`sons_module_n6_recherche.json`.

Usage :  python3 generer_audio_module_n6_recherche.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n6-recherche"
MANIFESTE = RACINE / "sons_module_n6_recherche.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n6-recherche/dialogues.js"
...
# Trois personnages, trois voix. Djamila, la conseillère en emploi, prend la
# voix de l'enseignante — celle que `voix_lente` ralentit à 0,85 : c'est elle
# qui explique, et c'est elle qu'un élève doit pouvoir suivre mot à mot.
# Marisol et Robert ne partagent aucune voix avec elle : les trois se
# répondent dans les mêmes dialogues.
VOIX_PERSO = {
    "MARISOL": "feminin_2",
    "DJAMILA": "enseignante",
    "ROBERT":  "masculin_1",
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"lancer build/collecte_sons.py module-n6-recherche")
        sys.exit(1)
    ...
    print(f"🔊 module-n6-recherche — {len(taches)} extraits "
