#!/usr/bin/env python3
"""
Générateur d'audio — module « Chercher un emploi en région » (niveau 7)
(module-n7-recherche).
...
`build/contenu/module-n7-recherche/dialogues.js`, la source unique.

Niveau 7, donc des **discours longs** : les quatre extraits font de dix-huit à
vingt et une répliques, et le reportage économique du Défi 1 en compte vingt et
une, dont plusieurs de quatre ou cinq phrases pleines de pourcentages. C'est ce
qui rend possibles les trois écoutes successives — une pour le sujet, une pour
les chiffres, une pour le détail — qu'aucun dialogue court ne permettrait.

Le relevé des sons (famille 2) n'a **pas** été fait par le navigateur ni par
`build/collecte_sons.py`, qu'il ne faut pas lancer :

    node build/releve_sons.js module-n7-recherche > sons_module_n7_recherche.json
...
Usage :  python3 generer_audio_module_n7_recherche.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n7-recherche"
MANIFESTE = RACINE / "sons_module_n7_recherche.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n7-recherche/dialogues.js"
...
# Six personnages pour quatre voix. Deux ne peuvent partager une voix que
# s'ils ne se répondent jamais dans un même extrait ; c'est vérifié ici :
#
#   prep  HAFIDA, SYLVAIN            t1  ODILE, GHISLAIN, FRÉDÉRICK
#   t2    HAFIDA, MARIE-ÈVE          t3  HAFIDA, MARIE-ÈVE
#
# HAFIDA et ODILE partagent `feminin_2` sans jamais se croiser — Hafida est
# absente du reportage, qu'elle écoute à la radio. SYLVAIN et FRÉDÉRICK
# partagent `masculin_1`, eux non plus.
#
# Le choix de qui prend `enseignante` n'est pas neutre : c'est la voix que
# `voix_lente` ralentit à 0,85. Elle va à Marie-Ève, la conseillère qui
# explique la méthode et relit le curriculum vitæ — exactement le rôle pour
# lequel un débit posé a été introduit. Odile, la journaliste, garde un débit
# normal : son reportage EST l'exercice d'écoute longue du module, et le
# ralentir en retirerait la difficulté que le niveau 7 vise.
VOIX_PERSO = {
    "HAFIDA":     "feminin_2",
    "SYLVAIN":    "masculin_1",
    "MARIE-ÈVE":  "enseignante",
    "ODILE":      "feminin_2",
    "GHISLAIN":   "narrateur",
    "FRÉDÉRICK":  "masculin_1",
}
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"lancer build/collecte_sons.py module-n7-recherche")
        sys.exit(1)
    ...
    print(f"🔊 module-n7-recherche — {len(taches)} extraits "
