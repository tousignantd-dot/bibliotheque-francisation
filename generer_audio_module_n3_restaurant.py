#!/usr/bin/env python3
"""
Générateur d'audio — module « Commander au comptoir » (module-n3-restaurant, niveau 3).
...
Les dialogues sont lus dans `build/contenu/module-n3-restaurant/dialogues.js` :
une seule source, comme aux niveaux 2 et 5.

Les identifiants de la famille 2 ne s'inventent pas — ils sont produits par le
moteur au rendu. `sons_module_n3_restaurant.json` a été produit hors navigateur
par `/private/tmp/releve_n3_restaurant.js`, qui rejoue sur les fichiers de
...
Usage :  python3 generer_audio_module_n3_restaurant.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n3-restaurant"
MANIFESTE = RACINE / "sons_module_n3_restaurant.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n3-restaurant/dialogues.js"
...
# moins un dialogue, donc aucune ne peut être partagée. Fatou explique le
# tableau du menu à Yolette : elle prend la voix « enseignante », ralentie à
# 0,85 par voix_lente.py.
VOIX_PERSO = {
    "YOLETTE": "feminin_2",
    "FATOU":   "enseignante",
    "STEVE":   "masculin_1",
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"relancer node /private/tmp/releve_n3_restaurant.js")
        sys.exit(1)
    ...
    print(f"🔊 module-n3-restaurant — {len(taches)} extraits "
