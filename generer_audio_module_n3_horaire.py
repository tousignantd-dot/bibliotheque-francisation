#!/usr/bin/env python3
"""
Générateur d'audio — module « Mon quart de travail » (module-n3-horaire,
niveau 3, activité 84).
...
**277 extraits attendus** : 72 répliques sur cinq dialogues, et 205 sons.

Les dialogues sont lus dans `build/contenu/module-n3-horaire/dialogues.js` :
une seule source, comme aux niveaux 2, 3 et 5.

Les identifiants de la famille 2 ne s'inventent pas — ils sont produits par le
moteur au rendu. `sons_module_n3_horaire.json` a été produit **hors
navigateur**, par :

    node build/releve_sons.js module-n3-horaire > sons_module_n3_horaire.json
...
jamais trouvée — la pastille lirait alors le mot seul, mal prononcé. Les 45
clés de ce module ont été relevées par script sur `exos.js`, et
`node build/coherence.js module-n3-horaire` le revérifie dans les deux sens.

**État au 22 août 2026 : aucun extrait n'a encore été produit.** Le module est
livré muet — les MP3 coûtent de l'argent réel et se rattrapent d'un seul coup :
...
Usage :  python3 generer_audio_module_n3_horaire.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n3-horaire"
MANIFESTE = RACINE / "sons_module_n3_horaire.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n3-horaire/dialogues.js"
...
# Trois personnages, trois voix distinctes : Fabiola, Gaétan et Miguel se
# répondent dans le même dialogue d'ouverture, donc aucune ne peut être
# partagée. Fabiola garde la voix de l'enseignante — c'est elle qui porte le
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — le produire par "
              f"« node build/releve_sons.js module-n3-horaire > "
    ...
    print(f"🔊 module-n3-horaire — {len(taches)} extraits "
