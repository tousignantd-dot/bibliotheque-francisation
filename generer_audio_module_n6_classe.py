"""
Générateur d'audio — module « Un travail de recherche en équipe »
(module-n6-classe, niveau 6, activité 107).

Deux familles de fichiers, comme dans les autres modules :

  1. les répliques des trois dialogues → <module>/<dialId>/line_NN_<perso>.mp3
  2. les mots, phrases et mini-leçons → <module>/sons/<fileId>.mp3

**217 extraits attendus** : 62 répliques sur trois dialogues, et 155 sons.

Les dialogues sont lus dans `build/contenu/module-n6-classe/dialogues.js` :
...
moteur au rendu. `sons_module_n6_classe.json` a été produit **hors
navigateur**, par :

    node build/releve_sons.js module-n6-classe > sons_module_n6_classe.json
...
Les dialogues sont longs : dix-neuf à vingt et une répliques chacun, et
plusieurs répliques de l'enseignante dépassent cinquante mots — une consigne
...
**Quatre personnages pour trois timbres** — deux personnages ne partagent un
timbre que s'ils ne se répondent jamais, c'est vérifié :

    prep  MIREILLE, MARISOL, YOUSSEF
    t1    MIREILLE, MARISOL, YOUSSEF
    t2    DANIÈLE, MARISOL, YOUSSEF

MIREILLE et DANIÈLE partagent `enseignante` sans jamais se croiser : la
première tient les deux premiers dialogues, la seconde le troisième, et elles
ne paraissent pas ensemble. C'est un choix plutôt qu'un pis-aller — ce sont les
deux voix qui expliquent, et un débit posé y est un avantage. La voix
« enseignante » est ralentie à 0,85 par `voix_lente.py` ; le paramètre `speed`
d'ElevenLabs ne fonctionne pas avec `eleven_multilingual_v2`. MARISOL et
YOUSSEF gardent un débit normal : au niveau 6, la compétence vise le débit
normal.
...
muet, et le son se rattrape d'un seul coup :
    ...
    python3 generer_audio_module_n6_classe.py
...
Usage :  python3 generer_audio_module_n6_classe.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n6-classe"
MANIFESTE = RACINE / "sons_module_n6_classe.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n6-classe/dialogues.js"
...
def slug(nom):
    ...
    normaliserait tout : charSlug garde le trait d'union, et d'autres modules
    ont bien un `line_06_marc-olivier.mp3`.
    """
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — le produire par "
              f"« node build/releve_sons.js module-n6-classe > "
