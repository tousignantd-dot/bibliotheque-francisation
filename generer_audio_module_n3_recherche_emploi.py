#!/usr/bin/env python3
"""
Générateur d'audio — module « On embauche » (module-n3-recherche-emploi,
niveau 3, activité 83).
...
**285 extraits attendus** : 66 répliques sur quatre dialogues, et 219 sons.

Les dialogues sont lus dans `build/contenu/module-n3-recherche-emploi/dialogues.js` :
une seule source, comme aux niveaux 2, 3 et 5.

Les identifiants de la famille 2 ne s'inventent pas — ils sont produits par le
moteur au rendu. `sons_module_n3_recherche_emploi.json` a été produit **hors
navigateur**, par :

    node build/releve_sons.js module-n3-recherche-emploi > sons_module_n3_recherche_emploi.json
...
jamais trouvée — la pastille lirait alors le mot seul, mal prononcé. Les 51
clés de ce module ont été relevées par script, et `node build/coherence.js
module-n3-recherche-emploi` le revérifie dans les deux sens.
...
muet, et le son se rattrape d'un seul coup :
    ...
    python3 generer_audio_module_n3_recherche_emploi.py
...
Usage :  python3 generer_audio_module_n3_recherche_emploi.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n3-recherche-emploi"
MANIFESTE = RACINE / "sons_module_n3_recherche_emploi.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n3-recherche-emploi/dialogues.js"
...
# Quatre personnages, quatre voix distinctes. Fanta parle dans les quatre
# dialogues et répond tour à tour à chacun des trois autres : sa voix ne peut
# donc être partagée avec personne. Sylvie, l'agente du centre d'emploi, prend
# celle de l'enseignante — c'est elle qui explique, et c'est la même voix qui
# porte les mots isolés et les mini-leçons.
VOIX_PERSO = {
    "SYLVIE": "enseignante",
    "FANTA":  "feminin_2",
    "GILLES": "masculin_1",
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — le produire par "
              f"« node build/releve_sons.js module-n3-recherche-emploi > "
    ...
    print(f"🔊 module-n3-recherche-emploi — {len(taches)} extraits "
