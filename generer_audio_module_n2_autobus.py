#!/usr/bin/env python3
"""
Générateur d'audio — module « Prendre l'autobus » (niveau 2) (module-n2-autobus).
...
`build/contenu/module-n2-autobus/dialogues.js`, la source unique.
...
`build/collecte_sons.py`, qui écrit `sons_module_n2_autobus.json`.

Usage :  python3 generer_audio_module_n2_autobus.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n2-autobus"
MANIFESTE = RACINE / "sons_module_n2_autobus.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n2-autobus/dialogues.js"
...
# Quatre personnages pour quatre voix. Marie-Josée, la passante qui explique
# le chemin, prend la voix de l'enseignante — celle que `voix_lente` ralentit
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"lancer build/collecte_sons.py module-n2-autobus")
        sys.exit(1)
    ...
    print(f"🔊 module-n2-autobus — {len(taches)} extraits "
