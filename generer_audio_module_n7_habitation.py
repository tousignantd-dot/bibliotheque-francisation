"""
Générateur d'audio — module « Le bruit d'en haut »
(module-n7-habitation, niveau 7, activité 112).
...
**323 extraits attendus** : 104 répliques sur quatre dialogues, et 219 sons.

Les dialogues sont lus dans `build/contenu/module-n7-habitation/dialogues.js` :
...
moteur au rendu. `sons_module_n7_habitation.json` a été produit **hors
navigateur**, par :

    node build/releve_sons.js module-n7-habitation > sons_module_n7_habitation.json
...
**Les mots isolés passent par `enrichir()` de `build/voix.py`.** Rien de ce
...
**Cinq personnages, quatre timbres** :
    ...
    prep  RUSLANA, MARLÈNE            t1  RUSLANA, CÉDRIC
    t2    RUSLANA, GINETTE, HUBERT    t3  RUSLANA, HUBERT

MARLÈNE (la collègue, Je découvre) et GINETTE (la propriétaire, défi 2)
...
ces deux rôles-là : ce sont eux qui énoncent la règle des inconvénients
normaux et les obligations du locateur.

**Pourquoi le médiateur est un homme, et pourquoi ça compte ici.** Le dépôt ne
dispose que de **deux** voix féminines (`enseignante` et `feminin_2`). Or le
dialogue du défi 2 fait parler trois personnes à la suite : la locataire, la
propriétaire, puis le service de médiation. À trois femmes, deux d'entre elles
auraient eu le même timbre **dans le même dialogue**, et l'élève n'aurait pas
pu les distinguer à l'écoute — ce qui ruinerait l'exercice, puisque tout le
défi porte sur qui a dit quoi. Hubert Vachon a donc la voix `narrateur`, la
quatrième et dernière du jeu. Faire du propriétaire un homme aurait été
l'autre solution ; elle a été écartée parce que `module-n7-logement`, le
module voisin du même niveau, a déjà un propriétaire masculin.
...
muet, et le son se rattrape d'un seul coup :
    ...
    python3 generer_audio_module_n7_habitation.py
...
Usage :  python3 generer_audio_module_n7_habitation.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n7-habitation"
MANIFESTE = RACINE / "sons_module_n7_habitation.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n7-habitation/dialogues.js"
...
# Cinq personnages, quatre timbres. MARLÈNE (Je découvre) et GINETTE (défi 2)
# partagent « enseignante » : elles ne se répondent jamais et ne paraissent
# jamais dans le même dialogue. HUBERT prend `narrateur` parce que le dialogue
# du défi 2 fait parler trois personnes à la suite et que le dépôt n'a que
# deux voix féminines — voir le docstring.
VOIX_PERSO = {
    "RUSLANA": "feminin_2",
    "MARLÈNE": "enseignante",
    "CÉDRIC":  "masculin_1",
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — le produire par "
              f"« node build/releve_sons.js module-n7-habitation > "
    ...
    print(f"🔊 module-n7-habitation — {len(taches)} extraits "
