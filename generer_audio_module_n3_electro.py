#!/usr/bin/env python3
"""
Générateur d'audio — module « Acheter un appareil » (module-n3-electro, niveau 3).
...
Les dialogues sont lus dans `build/contenu/module-n3-electro/dialogues.js` :
une seule source, comme aux niveaux 2 et 5.

Les identifiants de la famille 2 ne s'inventent pas — ils sont produits par le
moteur au rendu. `sons_module_n3_electro.json` a été produit hors navigateur
par `/tmp/releve_electro.js`, qui rejoue sur les fichiers de contenu les trois
...
Usage :  python3 generer_audio_module_n3_electro.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n3-electro"
MANIFESTE = RACINE / "sons_module_n3_electro.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n3-electro/dialogues.js"
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"lancer build/collecte_sons.py module-n3-electro")
        sys.exit(1)
    ...
    print(f"🔊 module-n3-electro — {len(taches)} extraits "
