#!/usr/bin/env python3
"""
Générateur d'audio — module « Faire l'épicerie » (module-alimentation).
...
`build/contenu/module-alimentation/dialogues.js`, la source unique.
...
`build/collecte_sons.py`, qui écrit `sons_module_alimentation.json`.

Usage :  python3 generer_audio_module_alimentation.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-alimentation"
MANIFESTE = RACINE / "sons_module_alimentation.json"
DIALOGUES_JS = RACINE / "build/contenu/module-alimentation/dialogues.js"
...
# Quatre personnages pour quatre voix. Johanne sert aux deux comptoirs (poisson
# et boucherie) ; Claude n'apparaît que dans les dialogues sur les étiquettes,
# jamais avec Simon.
VOIX_PERSO = {
    "FARIDA":  "feminin_2",
    "JOHANNE": "enseignante",
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — "
              f"lancer build/collecte_sons.py module-alimentation")
        sys.exit(1)
    ...
    print(f"🔊 module-alimentation — {len(taches)} extraits "
