#!/usr/bin/env python3
"""
Générateur d'audio — module « J'envoie une lettre et un colis » (niveau 2)
(module-n2-colis).
...
`build/contenu/module-n2-colis/dialogues.js`, la source unique.

Les identifiants de la famille 2, eux, ne s'inventent toujours pas : ils
viennent du moteur, au rendu. **Ils se relèvent hors du navigateur**, avec
`node build/releve_sons.js module-n2-colis > sons_module_n2_colis.json` —
et jamais avec `build/collecte_sons.py`, dont le port peut être occupé par une
session voisine : le relevé part alors écrire dans le fichier d'un autre
module sans que rien ne le dise.

Ce script n'a pas encore été lancé : les clés d'API n'étaient pas à la
disposition de la session qui a produit le module. Les 168 extraits du
relevé et les 63 répliques des six dialogues — 231 en tout — sont donc en
attente, et le script est relançable tel quel — il saute ce qui existe déjà.

Usage :  python3 generer_audio_module_n2_colis.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n2-colis"
MANIFESTE = RACINE / "sons_module_n2_colis.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n2-colis/dialogues.js"
...
# Trois personnages pour trois voix, et aucune n'est partagée : le module est
# court, ses dialogues n'ont jamais plus de deux voix à la fois.
#
# Amara Diallo est l'élève du module ; Luc Tremblay tient le comptoir postal ;
# Karim Haddad est son camarade de classe. Luc et Karim ne se répondent jamais
# — Luc ne parle que dans `prep`, `t1b` et `t2`, Karim que dans `t1`, `t2b` et
# `appli` — mais ils gardent tout de même deux voix distinctes : l'élève passe
# de l'un à l'autre d'un onglet au suivant, et rien ne l'avertit du changement
# sinon la voix.
VOIX_PERSO = {
    "AMARA": "feminin_2",
    "LUC":   "masculin_1",
...
def main():
    ...
    if not MANIFESTE.exists():
        print(f"❌ {MANIFESTE.name} introuvable — lancer\n"
              f"   node build/releve_sons.js module-n2-colis > {MANIFESTE.name}")
        sys.exit(1)
    ...
    print(f"🔊 module-n2-colis — {len(taches)} extraits "
