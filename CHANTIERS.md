...
| `fiches-autonomes` | dépôt principal | `build/powerpoints/fiche.py`, `build/powerpoints/theme.py`, `build/powerpoints/decks/module-n7-emploi/{a4,c4,d1}.py`, `build/powerpoints/decks/module-n6-recherche/b1.py`, `build/audit_renvois.py`, `assets/presentations/renvois-des-fiches.html`, les 1 265 fiches et les PowerPoints des deux modules touchés | **fait — 7 septembre 2026.** Règle posée : *la fiche de l'élève est autonome*. Gabarit `document()` dans les **deux** moteurs — la diapositive montre le document, la fiche le porte : ordre du jour, programme de prévention + articles 12-13, note de service, offre de Boisverte. Et `_sans_ecran()` dans `esc()` : la feuille ne dit plus « diapositive précédente » (179 fiches). Le relevé devient un contrôle, à garder à zéro. |
| `demo-seance` | dépôt principal | `build/demo_seance.py`, `.gitignore` | **fait — 7 septembre 2026.** Une séance sans compte peuplée pour la présentation : code `PROB24`, vingt participants anonymes sur le module 10 du niveau 4, 1 152 réponses tirées d'un modèle semé (aisance par personne × difficulté par zone). Les zones sont **lues dans le module** par node, jamais recopiées. Ni oral ni écrit : le mode n'en collecte pas, une démonstration qui en montrerait mentirait. `--install` / `--purge` / `--etat`. Fiche au classeur, famille « Présenter » : `assets/presentations/demo-seance-sans-compte.html` — le déroulé en quatre gestes et les deux captures. |
| `film-et-tutoriels` | dépôt principal | `build/teaser_pseudonymes.py`, `build/tutoriels/livrer.py`, `build/tutoriels/manifeste.json`, `assets/tutoriels/teaser-francis*.mp4`, `assets/outils/tutoriels-enseignant.html` | **fait — 7 septembre 2026.** Second export du film remis par l'utilisateur : 104 s, 1280×720, et **les six noms d'élèves de retour**. Le repaint est réétalonné — `px()` ramène chaque longueur à la hauteur du film, la fenêtre est en fractions de l'image, et `trouver_le_plan()` cherche le tableau tout seul. Deux défauts latents depuis le 1080 corrigés : la légende effacée sans être réécrite, et la gouttière non mesurable pendant un fondu. Vérifié image par image à contraste ×4. La page des tutoriels passe en vignettes cliquables. **Le son reste coupé à 8 kHz : c'est en amont du montage.** |
| `courriel-de-presentation` | dépôt principal | `build/captures_courriel.py`, `assets/presentations/courriel-presentation.html`, `assets/presentations/captures-courriel/`, `presentations.html`, `build/tutoriels/livrer.py`, `assets/outils/tutoriels-enseignant.html` | **fait — 9 septembre 2026.** Un **second** courriel aux directions, à côté de celui de démarchage : celui-ci **montre** au lieu de faire cliquer. Quatre points — parler sans témoin, le cours dans un téléphone, la classe vue pendant qu'elle travaille, les 90 ateliers autour des cours — les trois premiers suivis de l'image qui les prouve. `build/captures_courriel.py` recadre les captures existantes pour un courriel : bande horizontale, 560 px, **deux écrans de téléphone par fichier**, 119 Ko en tout, et le texte de remplacement de chacune est écrit sur la page. Les deux courriels ne visent pas la même situation — à froid, une seule image ; en réponse à « racontez-moi », quatre. Au passage : les sous-titres des capsules de tutoriels ne s'affichent plus d'eux-mêmes. |
...
| `dea-verdict-et-banc` | dépôt principal (+ `portfolio-conception`) | `presentations.html`, `assets/presentations/{formation-dea,dea-verdict}.html`, `assets/presentations/medias/formation-dea/`, `assets/presentations/atelier-prototypes/{banc-de-panne,banc-spade}.html` et `atelier-prototypes/banc-de-panne/` | **fait et poussé — 22 septembre 2026.** Trois chantiers. (1) La planche des rythmes du module DEA porte enfin le verdict — pastille « choc requis / pas de choc » dans l'écran, geste sur le papier en encre noire, et chaque rythme NOMMÉ en tête ; cinq narrations refaites, clés `t5-s1`→`t5-s5`, cinq anciens clips retirés. (2) Deux onglets à un seul lien en tête du classeur, **SimDEA** et **Simulateur barista**, engendrés par `portfolio-conception/onglets_simulateurs.py` entre marqueurs : la date est lue dans l'historique de la page SERVIE et le script **refuse de s'exécuter** si elle a des changements non commités. À relancer après chaque publication ET son commit. (3) Le banc de panne devient **« Avant l'ouverture »** — carnet en relevés qui nomment ce qu'ils retirent, chiffres du débriefing en blocs de statistique, quatrième habillage **Serre** aux verts mesurés sur spade.com, et la leçon « Avant le banc » renommée « Ce qui peut lâcher ». Deux défauts anciens trouvés en passant : la vue conception fuyait vers tous les joueurs (`.ligne .bits` pesait 0-2-0 contre 0-1-0), et dix médias existaient sur le disque sans avoir jamais été cuits dans la page servie. |
| `maison-francoeur` | dépôt principal | `build/contenu/entreprise-francoeur/`, `build/francoeur_*.py`, `assets/interactive/francoeur/`, `assets/presentations/francoeur-*.html`, `presentations.html` (une fiche), `server.py` (scénario « magasin », palier du jeu, voix par personnage), `data/activities.json` (activité 237) | **en cours — 24 septembre 2026.** **fait — 24 septembre 2026.** Les sept étapes du plan sont construites et poussées (lexique, planches, exercices, test, magasin, outillage du pilote, emballage). `server.py` et `data/activities.json` rendus. Reste hors code : le pilote réel, la relecture des langues, le crédit API pour les traductions en attente. |
