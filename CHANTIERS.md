...
| `micro-avant-la-voix` | dépôt principal | `build/gabarit/module.html`, `build/greffe_micro_abort.py`, les 78 modules à jeu de rôle | **fait — 7 septembre 2026.** `abort()` au lieu de `stop()` : le micro se rendait à `onend`, une seconde après, et le déclic de Chrome tombait sur le premier mot de l'assistant. `JR.sourd` se lève maintenant à la fermeture et retombe à la réouverture. |
| `fiches-plus` | dépôt principal | `build/fiches_plus.py`, `assets/documents/plus/`, `assets/presentations/fiches-plus-niveau-6.html`, `presentations.html` | **fait — 7 septembre 2026, non poussé.** Une seconde série des 160 fiches du niveau 6, avec la mini-leçon imprimée dans la fiche de sa séance, et la page à deux onglets pour comparer. `assets/documents/` n'est pas touché : la série d'essai vit dans son sous-dossier. |
| `bornage-par-niveau` | dépôt principal | `server.py` (bornage par niveau), `reseau.html`, `catalogue.html`, `build/controles/niveaux_acces.py` | **fait — 7 septembre 2026, non poussé.** Le champ `niveaux` sur la ligne d'accès : une personne bornée au niveau 6 y a tout, et rien ailleurs. Quatre points d'application, un seul texte de refus, et le trou de `?catalogue=1` bouché. Éprouvé au bac d'essai (26 vérifications) et le contrôle vu échouer sur ses quatre écarts. Détail : `CLAUDE.md`, section « Le bornage par niveau ». |
