...
| `maison-francoeur` | dépôt principal | `build/contenu/entreprise-francoeur/`, `build/francoeur_*.py`, `assets/interactive/francoeur/`, `assets/presentations/francoeur-*.html`, `presentations.html` (une fiche), `server.py` (scénario « magasin », palier du jeu, voix par personnage) | **en cours — 24 septembre 2026.** Étapes 0 à 4 poussées (… test de niveau, jeu de rôle en magasin : 8 clients × 4 humeurs). `server.py` rendu. Suite : étape 5, le pilote. |
