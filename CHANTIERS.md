...
| `maison-francoeur` | dépôt principal | `build/contenu/entreprise-francoeur/`, `build/francoeur_*.py`, `assets/interactive/francoeur/`, `assets/presentations/francoeur-*.html`, `presentations.html` (une fiche), `server.py` (scénario « magasin », palier du jeu, voix par personnage), `data/activities.json` (activité 237) | **en cours — 24 septembre 2026.** Étapes 0 à 5 poussées : voix en Azure HD, trousse au catalogue (activité 237), rapport au direct, protocole du pilote. `server.py` rendu. Suite : le pilote réel, puis l’étape 6. |
