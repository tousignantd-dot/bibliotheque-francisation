#!/usr/bin/env python3
"""
Générateur d'audio — module « Où est le local 214 ? » (niveau 2) (module-n2-couloirs).
...
`build/contenu/module-n2-couloirs/dialogues.js`, la source unique.
...
`node build/releve_sons.js module-n2-couloirs > sons_module_n2_couloirs.json`.
...
Usage :  python3 generer_audio_module_n2_couloirs.py [--force] [--only prefixe,...]
"""
import json
import os
import re
...
SORTIE = RACINE / "assets/interactive/module-n2-couloirs"
MANIFESTE = RACINE / "sons_module_n2_couloirs.json"
DIALOGUES_JS = RACINE / "build/contenu/module-n2-couloirs/dialogues.js"
...
# Quatre personnages pour quatre voix. Amina prend celle de l'enseignante,
# la seule que `voix_lente` ralentit à 0,85 : c'est la personne qui arrive au
# centre et qui hésite, et son débit lent est exactement ce que l'élève doit
# entendre au moment où il devient, lui, celui qui renseigne. Gilles et
# Vincent se répondent dans des dialogues différents mais gardent deux voix
# masculines distinctes : le concierge et l'élève ne doivent pas se confondre.
VOIX_PERSO = {
    "SORAYA":  "feminin_2",
    "GILLES":  "masculin_1",
    "VINCENT": "narrateur",
...
def main():
    ...
    print(f"🔊 module-n2-couloirs — {len(taches)} extraits "
